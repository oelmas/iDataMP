...
namespace MissionPlanner.Controls.PreFlight
{
    public partial class CheckListEditor : Form
    {
        ...
        private CheckListInput addwarningcontrol(int x, int y, CheckListItem item, bool hideforchild = false)
        {
            var wrnctl = new CheckListInput(_parent, item);
            ...
            if (item.Child != null) wrnctl = addwarningcontrol(x += 5, y, item.Child, true);

            return wrnctl;
        }

        private void wrnctl_ChildAdd(object sender, EventArgs e)
        {
            reload();
        }
