...
namespace MissionPlanner.Controls
{
    public partial class MovingBase : Form
    {
        ...
        private static MovingBase Instance;
        private static TcpListener listener;
        private static ICommsSerial comPort = new SerialPort();
        internal static PointLatLngAlt lastgotolocation = new PointLatLngAlt(0, 0, 0, "Goto last");
        internal static PointLatLngAlt gotolocation = new PointLatLngAlt(0, 0, 0, "Goto");
        internal static int intalt = 100;
        private static float updaterate = 0.5f;
        private static bool updaterallypnt;
        private Thread t12;
        ...
        {
            var nextsend = DateTime.Now;

            var nextrallypntupdate = DateTime.Now;

            var sw = new StreamWriter(Settings.GetUserDataDirectory() + "MovingBase.txt");
            ...
            while (threadrun)
                try
                {
                    ...
                    if (DateTime.Now > nextsend && gotolocation.Lat != 0 &&
                        gotolocation.Lng != 0) // 200 * 10 = 2 sec /// lastgotolocation != gotolocation && 
                    {
                        ...
                        if (updaterallypnt && DateTime.Now > nextrallypntupdate)
                        {
                            ...
                            try
                            {
                                MainV2.comPort.setParam((byte)MainV2.comPort.sysidcurrent,
                                    (byte)MainV2.comPort.compidcurrent, "RALLY_TOTAL", 1);

                                MainV2.comPort.setRallyPoint(0,
                                    new PointLatLngAlt(gotolocation)
                                    {
                                        Alt =
                                            gotolocation.Alt + double.Parse(Settings.Instance["TXT_DefaultAlt"])
                                    },
