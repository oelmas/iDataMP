...
namespace MissionPlanner.Controls
{
    public partial class MavCommandSelection : Form
    {
        ...
        private void loadFromSettings()
        {
            ...
            try
            {
                commands = JsonConvert.DeserializeObject<Dictionary<string, string[]>>(
                    Settings.Instance["PlannerExtraCommand"]);
                extraCommands =
                    JsonConvert.DeserializeObject<Dictionary<string, ushort>>(
                        Settings.Instance["PlannerExtraCommandIDs"]);
            }
            catch
            {
            }
            ...
            foreach (var cmd in commands)
                ...
                if (Enum.IsDefined(typeof(MAVLink.MAV_CMD), cmd.Key))
                {
                    ...
                    myDataGridView1.Rows[selectedrow].Cells[msgid.Index].Value =
                        ((ushort)Enum.Parse(typeof(MAVLink.MAV_CMD), cmd.Key)).ToString();
        ...
        private bool checkIDandName(ushort cmdId, string cmdName)
        {
            ...
            for (var i = 0; i < myDataGridView1.RowCount; i++)
            {
                var cmdNameRow = myDataGridView1.Rows[i].Cells[msgname.Index].Value.ToString();
                ...
                var cmdIdRow = ushort.Parse(myDataGridView1.Rows[i].Cells[msgid.Index].Value.ToString());
                if (cmdIdRow == cmdId)
                {
                    retval = true;
                    break;
        ...
        private void btn_AddLine_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (cmdid != "-1")
                {
                    var i = ushort.Parse(cmdid);
                    var a = ((MAVLink.MAV_CMD)i).ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            for (var i = 0; i < myDataGridView1.RowCount; i++)
                try
                {
                    var cmdName = myDataGridView1.Rows[i].Cells[msgname.Index].Value.ToString();
                    var cmdId = ushort.Parse(myDataGridView1.Rows[i].Cells[msgid.Index].Value.ToString());
                    ...
                    if (!Enum.IsDefined(typeof(MAVLink.MAV_CMD), cmdId)) extraCommands.Add(cmdName, cmdId);
                }
                catch
                {
                }
