...
namespace MissionPlanner.Controls
{
    public class ProximityControl : Form
    {
        ...
        private void Temp_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (var x = 50f; x <= screenradius; x += 50f)
            {
                var location = new Vector3(0, x / scale);
                ...
                e.Graphics.DrawArc(Pens.DimGray, (float)(midx - length), (float)(midy - length),
                    ...
                    360);
                e.Graphics.DrawString((x / 100).ToString("0.0m"), font, Brushes.Green, midx - (float)location.x + move,
                    midy - (float)location.y);
            }

            for (float x = 0; x < 360; x += 45f)
            {
                var location = new Vector3(0, screenradius / scale);
                var doublelength = location.length() * 2.0f;
                var length = location.length();

                location.rotate(x);
                e.Graphics.DrawString(x.ToString("0"), font, Brushes.DimGray, midx - (float)location.x - move * 2,
                    midy - (float)location.y + move);
                e.Graphics.DrawLine(Pens.DimGray, midx, midy, midx - (float)location.X,
                    midy - (float)location.Y);
            }
            ...
            {
                var location = new Vector3(0, Math.Min(temp.Distance / scale, screenradius / scale));
                ...
                switch (temp.Orientation)
                {
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_NONE:
                        ...
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x - move * 2, midy - (float)location.y + move);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, -22.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_45:
                        location.rotate(Rotation.ROTATION_YAW_45);
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x - move * 8, midy - (float)location.y + move);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 22.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_90:
                        ...
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x - move * 8, midy - (float)location.y);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 67.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_135:
                        ...
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x - move * 8, midy - (float)location.y - move);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 112.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_180:
                        location.rotate(Rotation.ROTATION_YAW_180);
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x - move * 2, midy - (float)location.y - move * 3);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 157.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_225:
                        location.rotate(Rotation.ROTATION_YAW_225);
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x + move, midy - (float)location.y - move);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 202.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_270:
                        location.rotate(Rotation.ROTATION_YAW_270);
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x + move, midy - (float)location.y);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 247.5f - 90f, 45f);
                        break;
                    case MAV_SENSOR_ORIENTATION.MAV_SENSOR_ROTATION_YAW_315:
                        ...
                        e.Graphics.DrawString((temp.Distance / 100).ToString("0.0m"), font, Brushes.Green,
                            midx - (float)location.x + move, midy - (float)location.y);
                        e.Graphics.DrawArc(redpen, (float)(midx - length), (float)(midy - length), (float)doublelength,
                            (float)doublelength, 292.5f - 90f, 45f);
                        break;
        ...
        private void InitializeComponent()
        {
            ...
            // ProximityControl
            // 
            ClientSize = new Size(430, 391);
            Name = "ProximityControl";
            ResumeLayout(false);
        }
    }
}