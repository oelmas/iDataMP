...
namespace MissionPlanner.Controls
{
    public partial class SerialOutputCoT : Form
    {
        ...
        private static ICommsSerial CoTStream = new SerialPort();
        private static double updaterate = 5;
        private static bool threadrun;
        internal static PointLatLngAlt HomeLoc = new PointLatLngAlt(0, 0, 0, "Home");
        private Thread t12;
        ...
        private void BUT_connect_Click(object sender, EventArgs e)
        {
            ...
            {
                threadrun = false;
                if (CoTStream != null && CoTStream.IsOpen) CoTStream.Close();
            ...
            try
            {
                switch (CMB_serialport.Text)
                {
                    ...
                    case "TCP Host":
                        ...
                        listener = new TcpListener(IPAddress.Any, 14551);
                        listener.Start(0);
            ...
            {
                if (listener == null)
                    ThreadPool.QueueUserWorkItem(background_DoOpen);
            }
            catch
            {
        ...
        private void background_DoOpen(object state)
        {
            if (CoTStream == null) return;
        ...
        private void mainloop()
        {
            ...
            while (threadrun)
                try
                {
                    ...
                    TB_output.Invoke((Action)delegate { TB_output.Text = xmlStr; });

                    if (CoTStream != null && CoTStream.IsOpen) CoTStream.WriteLine(xmlStr);
        ...
        private string getXmlString()
        {
            ...
            var xmlStr = getXmlString(TB_xml_uid.Text, TB_xml_type.Text, how, lat, lng, altitude, groundcourse,
                groundSpeed);

            return xmlStr;
        }

        private string getXmlString(string uid, string type, string how, double lat, double lng, double alt,
            double course = -1, double speed = -1)
        {
            ...
            var cotevent = new @event
            {
                ...
                point = new point
                {
                    lat = lat.ToString("N7", culture), lon = lng.ToString("N7", culture),
                    hae = alt.ToString("N2", culture).PadLeft(5, ' ')
                }
            };
            ...
            using (StringWriter textWriter = new Utf8StringWriter())
            {
                ...
                var xtw = XmlWriter.Create(textWriter, xws);
            ...
            sb.AppendLine(); // CoT spec section 2.3, additional values by MIL-STD-2525

            sb.AppendFormat("    time=\"{0}\"", time.ToString("o"));
            sb.AppendLine(); // time stamp: when the event was generated
            sb.AppendFormat("    start=\"{0}\"", time.AddSeconds(-5).ToString("o"));
            sb.AppendLine(); // starting time when an event should be considered valid
            sb.AppendFormat("    stale=\"{0}\"", time.AddSeconds(5).ToString("o"));
            sb.AppendLine(); // ending time when an event should no longer be considered valid
            ...
            // m-g-n    == h.ins-gps
            // m-g-d    == h.dgps
            sb.AppendFormat("    how=\"{0}\">", how);
            sb.AppendLine(); // Gives a hint about how the coordinates were generated


            sb.AppendLine("  <detail>");
            if (course >= 0 && course <= 360 && speed >= 0)
            {
                sb.AppendFormat(culture, "    <track course=\"{0:N2}\" speed=\"{1:N2}\" />", course, speed);
                sb.AppendLine();
            }
            ...
            sb.AppendFormat(culture, "  <point lat=\"{0:N7}\" lon=\"{1:N7}\" hae=\"{2,5:N2}\" ce=\"1.0\" le=\"1.0\"/>",
                lat, lng, alt);
            sb.AppendLine();
            sb.AppendLine("</event>");
        ...
        {
            var uid = Settings.Instance["SerialOutputCot_TB_xml_uid"];
            if (uid == null) uid = "";
