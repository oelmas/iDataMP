...
namespace MissionPlanner.Controls
{
    public partial class ThemeEditor : Form
    {
        public ThemeEditor()
        {
            ...
            foreach (var coloritem in ThemeManager.thmColor.colors)
        ...
        private void listboxThemeItems_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            lblColorName.Text = ThemeManager.thmColor.colors.Find(x => x.strColorItemName.Contains(lblItemName.Text))
                .clrColor.ToString();
            colorPatch.BackColor = ThemeManager.thmColor.colors.Find(x => x.strColorItemName.Contains(lblItemName.Text))
                .clrColor;
        }
        ...
        private void btnCopy_Click(object sender, EventArgs e)
        {
            ...
            var name = Path.GetFileNameWithoutExtension(ThemeManager.thmColor.strThemeName);

            if (DialogResult.Cancel ==
                InputBox.Show("Create Theme Copy", "Enter nem theme name (without extension)", ref name))
                return;

            name = name + ".mpusertheme";
            var match = ThemeManager.ThemeNames.FirstOrDefault(stringToCheck =>
                stringToCheck.Equals(name, StringComparison.OrdinalIgnoreCase));
            if (match != null)
            {
            ...
            var strFiletoSave = Settings.GetUserDataDirectory() + name;
            var tmpThemeName = ThemeManager.thmColor.strThemeName;
            try
            {
                ThemeManager.thmColor.strThemeName = name;
                ThemeManager.WriteToXmlFile(strFiletoSave, ThemeManager.thmColor);
                lblThemeName.Text = ThemeManager.thmColor.strThemeName;
            }
            catch
            {
        ...
        private void colorPatch_dblclick(object sender, MouseEventArgs e)
        {
            ...
            ThemeManager.thmColor.colors.Find(x => x.strColorItemName.Contains(lblItemName.Text)).clrColor =
        ...
        private void btnSaveApply_Click(object sender, EventArgs e)
        {
            ...
            var strFiletoSave = Settings.GetUserDataDirectory() + ThemeManager.thmColor.strThemeName;
            try
            {
                ThemeManager.WriteToXmlFile(strFiletoSave, ThemeManager.thmColor);
