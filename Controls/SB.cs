...
namespace MissionPlanner.Controls
{
    public class SB
    {
        public static void Show(string detectedvia)
        {
            ...
            var frm = new Form { Width = 250, Height = 250, AutoSize = true };
            var flp = new FlowLayoutPanel { Dock = DockStyle.Fill, FlowDirection = FlowDirection.TopDown };
            var lb1 = new Label { Text = l1, AutoSize = true };
            var link = new Label { Text = l2, AutoSize = true };


            var nameLabel = new Label { Text = "Enter your name (optional): ", AutoSize = true };
            var nameTextBox = new TextBox
                { Width = TextRenderer.MeasureText("thisismyname andmysurname", frm.Font).Width };
            var emailLabel = new Label { Text = "Enter your email (optional): ", AutoSize = true };
            var emailTextBox = new TextBox
                { Width = TextRenderer.MeasureText("thisisatest@thisdomain.com.fre.do", frm.Font).Width };
            var submitButton = new MyButton { Text = "Service Bulletin" };
