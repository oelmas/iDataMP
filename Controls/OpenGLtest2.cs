...
namespace MissionPlanner.Controls
{
    public class OpenGLtest2 : GLControl, IDeactivate
    {
        ...
        private readonly Vector3 _rpy = new Vector3();
        ...
        private OpenTK.Vector3 myrpy = OpenTK.Vector3.UnitX;
        private NumericUpDown num_maxzoom;
        private NumericUpDown num_minzoom;
        private bool onpaintrun;
        private readonly PureProjection prj;
        private Matrix4 projMatrix = Matrix4.Identity;
        private bool sizeChanged;
        private bool started;

        private readonly ConcurrentDictionary<GPoint, tileInfo>
            textureid = new ConcurrentDictionary<GPoint, tileInfo>();

        private readonly SemaphoreSlim textureSemaphore = new SemaphoreSlim(1, 1);
        private List<tileZoomArea> tileArea = new List<tileZoomArea>();
        private Timer timer1;
        private readonly GMapProvider type;
        ...
        public OpenGLtest2()
        {
            ...
            core.OnMapOpen();
            type = GoogleSatelliteMapProvider.Instance;
            prj = type.Projection;
        ...
        private PointLatLngAlt _center { get; } = new PointLatLngAlt(-34.9807459, 117.8514028, 70);
        ...
        public PointLatLngAlt getMousePos(int x, int y)
        {
            ...
            var _start = UnProject(new OpenTK.Vector3(x, y, 0.0f), projMatrix, modelMatrix,
                new Size(viewport[2], viewport[3]));
            var _end = UnProject(new OpenTK.Vector3(x, y, 1), projMatrix, modelMatrix,
                new Size(viewport[2], viewport[3]));
        ...
        public static OpenTK.Vector3 UnProject(OpenTK.Vector3 mouse, Matrix4 projection, Matrix4 view, Size viewport)
        {
            Vector4 vec;
        ...
        private static int generateTexture(Bitmap image)
        {
        ...
        private static void ConvertColorSpace(BitmapData _data)
        {
            // bgra to rgba
            var x = 0;
            var y = 0;
        ...
        private static int CreateTexture(BitmapData data)
        {
            var texture = 0;
            ...
            GL.TexImage2D(TextureTarget2d.Texture2D, 0, TextureComponentCount.Rgba, data.Width, data.Height, 0,
                OpenTK.Graphics.ES20.PixelFormat.Rgba, PixelType.UnsignedByte, data.Scan0);
        ...
        private static int ColorBGRA2RGBA(int x)
        {
            // bgra > rgba
            unchecked
            {
        ...
        private void imageLoader()
        {
            ...
            // shared context
            var win = new GameWindow(640, 480, Context.GraphicsMode);
            win.Visible = false;
            ...
            while (!IsDisposed)
            {
                ...
                if (_center.GetDistance(core.Position) > 30) core.Position = _center;
        ...
        private double[] convertCoords(PointLatLngAlt plla)
        {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Extensions.ProtectReentry(doPaint);
        }
        ...
        public void doPaint()
        {
            ...
            try
            {
                ...
                {
                    campos = mypos;
                    rpy = new Vector3(myrpy.X, myrpy.Y, myrpy.Z);
                    var input = Keyboard.GetState();
                    var speed = 1.5f;
                    var position = new OpenTK.Vector3((float)campos[0], (float)campos[1], (float)campos[2]);
                    var front = OpenTK.Vector3.Normalize(new OpenTK.Vector3(
                        (float)Math.Sin(MathHelper.Radians(rpy.Z)) * 1,
                        (float)Math.Cos(MathHelper.Radians(rpy.Z)) * 1, 0));
                    var up = new OpenTK.Vector3(0.0f, 0.0f, 1.0f);
                    if (input.IsKeyDown(Key.W)) position += front * speed; //Forward

                    if (input.IsKeyDown(Key.S)) position -= front * speed; //Backwards

                    if (input.IsKeyDown(Key.A))
                        position -= OpenTK.Vector3.Normalize(OpenTK.Vector3.Cross(front, up)) * speed; //Left

                    if (input.IsKeyDown(Key.D))
                        position += OpenTK.Vector3.Normalize(OpenTK.Vector3.Cross(front, up)) * speed; //Right

                    if (input.IsKeyDown(Key.Q)) rpy.Z -= speed;

                    if (input.IsKeyDown(Key.E)) rpy.Z += speed;

                    if (input.IsKeyDown(Key.R)) position.Z += speed / 2;

                    if (input.IsKeyDown(Key.F)) position.Z -= speed / 2;
                ...
                var lastzoom = texlist.Count == 0 ? 0 : texlist[0].Value.zoom;
                var beforedraw = DateTime.Now;
                foreach (var tidict in texlist)
                {
                    if (lastzoom != tidict.Value.zoom) lastzoom = tidict.Value.zoom;

                    if (tidict.Value.indices.Count > 0) tidict.Value.Draw(projMatrix, modelMatrix);
                }
                ...
                {
                    if (green == 0) green = generateTexture(Resources.green.ToBitmap());
                ...
                if (delta.TotalMilliseconds > 20)
                    Console.Write(
                        "OpenGLTest2 total {0} swap {1} wps {2} draw {3} clear {4} wait {5} bwait {6} wpmark {7}  \n",
        ...
        {
            var newloc = LocationProjection.Project(_center, Velocity, _centerTime, DateTime.Now);
        ...
        private void generateTextures()
        {
            ...
            lock (tileArea)
            {
                ...
                for (var a = minzoom; a <= zoom; a++)
                {
                    ...
                    {
                        var task = new LoadTask(p, a);
                        {
                            if (!core.tileLoadQueue.Contains(task)) core.tileLoadQueue.Push(task);
                        }
                    }
                }
            ...
            foreach (var tilearea in talist)
            {
                ...
                foreach (var p in tilearea.points)
                {
                    ...
                    var xstart = p.X * prj.TileSize.Width;
                    var ystart = p.Y * prj.TileSize.Width;
                    var xend = (p.X + 1) * prj.TileSize.Width;
                    var yend = (p.Y + 1) * prj.TileSize.Width;
                    try
                    {
                        var t = core.Matrix.GetTileWithNoLock(tilearea.zoom, p);
                        if (t.NotEmpty)
                            foreach (var imgPI in t.Overlays)
                            {
                                ...
                                if (!textureid.ContainsKey(p))
                                    try
                                    {
                                        var ti = new tileInfo(Context, WindowInfo, textureSemaphore)
                                        {
                                            point = p,
                                            zoom = tilearea.zoom,
                                            img = (Image)img.Img.Clone()
                                        };
                                        for (var x = xstart; x < xend; x += pxstep)
                                        {
                                            var xnext = x + pxstep;
                                            for (var y = ystart; y < yend; y += pxstep)
                                            {
                                                var ynext = y + pxstep;
        ...
        {
            foreach (var tileZoomArea in tileArea.Reverse<tileZoomArea>())
        ...
        private void InitializeComponent()
        {
            ...
            num_minzoom = new NumericUpDown();
            num_maxzoom = new NumericUpDown();
            chk_locktomav = new CheckBox();
            timer1 = new Timer(components);
            chk_fog = new CheckBox();
            ((ISupportInitialize)num_minzoom).BeginInit();
            ((ISupportInitialize)num_maxzoom).BeginInit();
            SuspendLayout();
            //
            // num_minzoom
            //
            num_minzoom.Location = new Point(3, 3);
            num_minzoom.Maximum = new decimal(new[]
            {
                20,
                0,
                0,
            ...
            num_minzoom.Minimum = new decimal(new[]
            {
                1,
                0,
                0,
            ...
            num_minzoom.Name = "num_minzoom";
            num_minzoom.Size = new Size(54, 20);
            num_minzoom.TabIndex = 0;
            num_minzoom.Value = new decimal(new[]
            {
                12,
                0,
                0,
            ...
            num_minzoom.ValueChanged += num_minzoom_ValueChanged;
            //
            // num_maxzoom
            //
            num_maxzoom.Location = new Point(3, 29);
            num_maxzoom.Maximum = new decimal(new[]
            {
                20,
                0,
                0,
            ...
            num_maxzoom.Minimum = new decimal(new[]
            {
                1,
                0,
                0,
            ...
            num_maxzoom.Name = "num_maxzoom";
            num_maxzoom.Size = new Size(54, 20);
            num_maxzoom.TabIndex = 1;
            num_maxzoom.Value = new decimal(new[]
            {
                20,
                0,
                0,
            ...
            num_maxzoom.ValueChanged += num_maxzoom_ValueChanged;
            //
            // chk_locktomav
            //
            chk_locktomav.AutoSize = true;
            chk_locktomav.Checked = true;
            chk_locktomav.CheckState = CheckState.Checked;
            chk_locktomav.Location = new Point(63, 6);
            chk_locktomav.Name = "chk_locktomav";
            chk_locktomav.Size = new Size(88, 17);
            chk_locktomav.TabIndex = 2;
            chk_locktomav.Text = "Lock to MAV";
            chk_locktomav.UseVisualStyleBackColor = true;
            //
            // timer1
            //
            timer1.Interval = 40;
            ...
            // chk_fog
            //
            chk_fog.AutoSize = true;
            chk_fog.Checked = true;
            chk_fog.CheckState = CheckState.Checked;
            chk_fog.Location = new Point(63, 32);
            chk_fog.Name = "chk_fog";
            chk_fog.Size = new Size(44, 17);
            chk_fog.TabIndex = 3;
            chk_fog.Text = "Fog";
            chk_fog.UseVisualStyleBackColor = true;
            chk_fog.CheckedChanged += chk_fog_CheckedChanged;
            //
            // OpenGLtest2
            //
            AutoScaleDimensions = new SizeF(6F, 13F);
            Controls.Add(chk_fog);
            Controls.Add(chk_locktomav);
            Controls.Add(num_maxzoom);
            Controls.Add(num_minzoom);
            Name = "OpenGLtest2";
            Size = new Size(640, 480);
            Load += test_Load;
            Resize += test_Resize;
            ((ISupportInitialize)num_minzoom).EndInit();
            ((ISupportInitialize)num_maxzoom).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }
        ...
        {
            if (onpaintrun && IsHandleCreated && !IsDisposed && !Disposing)
            {
                Invalidate();
                onpaintrun = false;
            }
    ...
    gl_FragColor = DestinationColor;
        ...
        public class tileInfo : IDisposable
        {
            ...
            public Image img
            {
                ...
                {
                    _img = value;
                    _data = ((Bitmap)_img).LockBits(new Rectangle(0, 0, _img.Width, _img.Height),
                        ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                }
            }
            ...
            public bool textureReady => _textid != 0;
...
void main(void) {
    ...
    gl_FragColor = color;// mix(color, vec4(0.4,0.6,0.9,1), fogAmount);
        ...
        public struct Vertex
        {
            ...
            public static readonly int Stride = Marshal.SizeOf(new Vertex());
        }
    }
}