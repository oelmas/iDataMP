...
...
                    ...
                    writeline(string.Format("#6,{0},{1},{2},{3},",
                        MainV2.comPort.MAV.cs.groundspeed *
                        Math.Sin(MainV2.comPort.MAV.cs.groundcourse * MathHelper.deg2rad),
                        MainV2.comPort.MAV.cs.groundspeed *
                        Math.Cos(MainV2.comPort.MAV.cs.groundcourse * MathHelper.deg2rad),
                        MainV2.comPort.MAV.cs.verticalspeed, 2));

                    writeline(string.Format("#7,{0},{1},{2},", MainV2.comPort.MAV.cs.roll * MathHelper.deg2rad,
                        MainV2.comPort.MAV.cs.pitch * MathHelper.deg2rad,
                        MainV2.comPort.MAV.cs.yaw * MathHelper.deg2rad));
        ...
        private DateTime GetFromGps(int weeknumber, int seconds)
        {
        ...
        private void GetGPSTime(out int weeknumber, out int seconds)
        {
            var datum = new DateTime(1980, 1, 6, 0, 0, 0, DateTimeKind.Utc);

            var ts = DateTime.Now - datum;
        ...
        private void GetDegrees(double x, double y, double z, out double latitude, out double longitude)
        {
        ...
        private void GetGeo(out double x, out double y, out double z, double latitude, double longitude, double alt)
        {
            double wgs84a = 6378137;
            var wgs84f = 1.0 / 298.257223563;
            var wgs84b = wgs84a * (1.0 - wgs84f);

            var clat = Math.Cos(MathHelper.deg2rad * latitude);
            var slat = Math.Sin(MathHelper.deg2rad * latitude);
            var clon = Math.Cos(MathHelper.deg2rad * longitude);
            var slon = Math.Sin(MathHelper.deg2rad * longitude);
        ...
        {
            byte ans = 0;
            foreach (var ch in line) ans += (byte)ch;
            ans = (byte)(ans ^ 0xff);
