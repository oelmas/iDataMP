...
namespace MissionPlanner.Controls
{
    public partial class DroneCANParams : MyUserControl, IActivate, IDeactivate
    {
        ...
        public DroneCANParams(DroneCAN.DroneCAN can, byte node,
        ...
        internal void processToScreen()
        {
            ...
            foreach (var item in _paramlist.Select(a => Encoding.ASCII.GetString(a.name, 0, a.name_len)))
                list.Add(item);
            ...
            Parallel.ForEach(list, value =>
            {
                ...
                row.Cells[Value.Index].Value = _paramlist
                    .First(a => Encoding.ASCII.GetString(a.name, 0, a.name_len) == value).value.GetValue();
                row.Cells[Min.Index].Value = _paramlist
                    .First(a => Encoding.ASCII.GetString(a.name, 0, a.name_len) == value).min_value.GetValue();
                row.Cells[Max.Index].Value = _paramlist
                    .First(a => Encoding.ASCII.GetString(a.name, 0, a.name_len) == value).max_value.GetValue();
                row.Cells[Default.Index].Value = _paramlist
                    .First(a => Encoding.ASCII.GetString(a.name, 0, a.name_len) == value).default_value.GetValue();
        ...
        private void BUT_load_Click(object sender, EventArgs e)
        {
                   ...
                   })
            {
                ...
                if (dr == DialogResult.OK) loadparamsfromfile(ofd.FileName, true);
            }
        }
        ...
        private void BUT_writePIDS_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in _changes.Keys) temp.Add((string)item);
            ...
            foreach (var value in temp)
                try
                {
                    ...
                    try
                    {
                        ...
                        if (textControls.Length > 0) ThemeManager.ApplyThemeTo(textControls[0]);
                    }
                    catch
                    {
                    }
                ...
                catch (Exception ex)
                {
                    log.Error(ex);
                    failed++;
                    CustomMessageBox.Show("Set " + value + " Failed " + ex);
                }
        ...
        private void filterList(string searchfor)
        {
            ...
            {
                var filter = new Regex(searchfor.Replace("*", ".*").Replace("..*", ".*"),
        ...
        {
            _filterTimer.Stop();
            Invoke((Action)delegate { filterList(txt_search.Text); });
        }
        ...
        private void loadparamsfromfile(string fn, bool offline = false)
        {
            ...
            foreach (var name in param2.Keys)
            {
                var set = false;
            ...
            {
                var list = "";
                foreach (var item in missing) list += item + " ";
                CustomMessageBox.Show("Missing " + missed + " params\n" + list, "No matching Params",
