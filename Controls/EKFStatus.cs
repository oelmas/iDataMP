...
namespace MissionPlanner.Controls
{
    public partial class EKFStatus : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            foreach (var item in new[] { ekfvel, ekfposh, ekfposv, ekfcompass, ekfterrain })
            {
                if (item.Value > 50)
            ...
            var idx = 0;
            for (var bitvalue = 1;
                 bitvalue <= (int)MAVLink.EKF_STATUS_FLAGS.EKF_UNINITIALIZED;
                 bitvalue = bitvalue << 1)
            {
                var currentbit = MainV2.comPort.MAV.cs.ekfflags & bitvalue;

                var currentflag =
                    (MAVLink.EKF_STATUS_FLAGS)Enum.Parse(typeof(MAVLink.EKF_STATUS_FLAGS), bitvalue.ToString());
