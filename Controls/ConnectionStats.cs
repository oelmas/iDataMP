...
namespace MissionPlanner.Controls
{
    public partial class ConnectionStats : UserControl
    {
            ...
            : this()
        {
            ...
            Load += ConnectionStats_Load;
            Disposed += (sender, e) => StopUpdates();
        }
        ...
        private void ConnectionStats_Load(object sender, EventArgs e)
        {
            ...
            var subscriptions = new List<IDisposable>
            {
                ...
                _mavlink.WhenPacketReceived
                    ...
                    .Subscribe(x => txt_PacketsPerSecond.Text = x.ToString("0")),
                ...
                CombineWithDefault(_mavlink.WhenPacketReceived, _mavlink.WhenPacketLost, Tuple.Create)
                    ...
                    .ObserveOn(SynchronizationContext.Current)
                    .Subscribe(x => txt_LinkQuality.Text = x.ToString("00%")),
...
//                                            .Subscribe(ts => this.txt_TimeConnected.Text = ts.ToString()),
            ...
            Subject<TSource> second, Func<TSource, TSource, TResult> resultSelector)
        {
            return Observable.Defer(() =>
            {
                ...
                first.Select(x => resultSelector(x, default)).Subscribe(foo);
                second.Select(x => resultSelector(default, x)).Subscribe(foo);
