...
namespace MissionPlanner.Controls
{
    ...
    {
        // for graph
        private readonly RollingPointPairList list1 = new RollingPointPairList(10 * 50);
        private readonly RollingPointPairList list2 = new RollingPointPairList(10 * 50);
        private readonly RollingPointPairList list3 = new RollingPointPairList(10 * 50);
        private readonly RollingPointPairList list4 = new RollingPointPairList(10 * 50);
        private readonly RollingPointPairList list5 = new RollingPointPairList(10 * 50);
        private readonly RollingPointPairList list6 = new RollingPointPairList(10 * 50);
        ...
        private void timer2serial_Tick(object sender, EventArgs e)
        {
            ...
            if (sw != null && sw.BaseStream.CanWrite)
                sw.WriteLine("{0},{1},{2},{3},{4},{5},{6}", DateTime.Now.ToString(), MainV2.comPort.MAV.cs.ax,
                    MainV2.comPort.MAV.cs.ay, MainV2.comPort.MAV.cs.az, MainV2.comPort.MAV.cs.gx,
                    MainV2.comPort.MAV.cs.gy, MainV2.comPort.MAV.cs.gz);

            var time = (Environment.TickCount - tickStart) / 1000.0;

            if (chkax.Checked)
                list1.Add(time, MainV2.comPort.MAV.cs.ax);
            else
                list1.Clear();
            if (chkay.Checked)
                list2.Add(time, MainV2.comPort.MAV.cs.ay);
            else
                list2.Clear();
            if (chkaz.Checked)
                list3.Add(time, MainV2.comPort.MAV.cs.az);
            else
                list3.Clear();
            if (chkgx.Checked)
                list4.Add(time, MainV2.comPort.MAV.cs.gx);
            else
                list4.Clear();
            if (chkgy.Checked)
                list5.Add(time, MainV2.comPort.MAV.cs.gy);
            else
                list5.Clear();
            if (chkgz.Checked)
                list6.Add(time, MainV2.comPort.MAV.cs.gz);
            else
                list6.Clear();
        }
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RAW_SENSORS,
                    MainV2.comPort.MAV.cs.ratesensors);
                // request raw sensor
        ...
        private void BUT_savecsv_Click(object sender, EventArgs e)
        {
            using (var ofd = new SaveFileDialog())
            {
                ...
                if (ofd.FileName != "" && result == DialogResult.OK)
                {
                    ...
                    sw = new StreamWriter(ofd.OpenFile());
                }
            }
        }
