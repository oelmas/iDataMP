...
namespace AltitudeAngelWings
{
    public static class ServiceLocator
    {
        ...
        private static readonly IDictionary<Type, Func<IServiceLocator, object>> ServiceRegistry =
            new Dictionary<Type, Func<IServiceLocator, object>>();

        public static T GetService<T>()
        {
            if (ServiceInstances.ContainsKey(typeof(T))) return (T)ServiceInstances[typeof(T)];
            lock (Lock)
            {
        ...
        public static void Configure(AppDomain appDomain = null)
        {
            ...
            foreach (var assembly in appDomain.GetAssemblies()) ConfigureFromAssembly(assembly);
        }
