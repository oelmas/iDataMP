...
namespace AltitudeAngelWings.ApiClient.Client
{
    public static class FeatureExtensions
    {
        ...
        {
            return JsonConvert.DeserializeObject<FeatureProperties>(
                new JObject(feature.Properties.Select(p => new JProperty(p.Key, p.Value))).ToString());
        }

        public static IEnumerable<FilterInfoDisplay> GetFilterInfo(this Feature feature)
        {
            if (!feature.Properties.ContainsKey("filters")) yield break;
        ...
        public static void UpdateFilterInfo(this IEnumerable<Feature> features,
            IList<FilterInfoDisplay> filterInfoToUpdate, bool resetFilters = false)
        {
            ...
            foreach (var change in features
                ...
                if (change.Update == null)
                {
                    ...
                    if (change.Item.Name == "Airspace") change.Item.Visible = false;
                }
                else
                {
                    ...
                    if (resetFilters) change.Update.Visible = change.Item.Name != "Airspace" && change.Item.Visible;
                }
        }

        public static DisplayInfo GetDisplayInfo(this Feature feature)
        {
            return JsonConvert.DeserializeObject<DisplayInfo>(feature.Properties["display"].ToString());
        }
    }
}