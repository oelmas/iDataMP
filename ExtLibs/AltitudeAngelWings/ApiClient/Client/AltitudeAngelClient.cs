...
namespace AltitudeAngelWings.ApiClient.Client
{
    public class AltitudeAngelClient : IAltitudeAngelClient
    {
        ...
        {
            return _asyncPolicy.ExecuteAsync(() => _settings.AuthenticationUrl
        ...
        public Task<StartFlightResponse> StartFlight(string flightPlanId)
        {
            var startFlightRequest = new StartFlightRequest
            {
                ...
                {
                    new TacticalDeconflictionFlightServiceRequest
                    {
            ...
            return _asyncPolicy.ExecuteAsync(() => _settings.FlightServiceUrl
                ...
                .WithClient(Client)
                .ConfigureRequest(settings => { settings.JsonSerializer = CreateNewtonsoftJsonSerializer(); })
        ...
        {
            return new NewtonsoftJsonSerializer(CreateJsonSerializerSettings());
        }
        ...
        private static CreateFlightPartRequest CreateFlightPartRequest(
            ...
            Feature geography)
        {
            return new CreateFlightPartRequest
            {
                Id = id.ToString(),
                Geography = geography,
                Start = start,
            ...
            UserProfileInfo currentUser)
        {
            ...
            if (_settings.FlightIdentifierIcao) identifiers.Add("icao", _settings.FlightIdentifierIcaoAddress);
            if (_settings.FlightIdentifierSerial)
                identifiers.Add("serialNumber", _settings.FlightIdentifierSerialNumber);
            if (identifiers.Count == 0) identifiers = null;

            var req = new CreateStrategicPlanRequest
            {
                ConflictResolutionScope = flightPlan.UseLocalConflictScope
                    ? StrategicConflictResolutionScope.Local
                    : StrategicConflictResolutionScope.Global,
        ...
        private static StrategicAirframeType MapToAirFrame(FlightCapability flightCapability)
        {
            switch (flightCapability)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException(nameof(flightCapability), flightCapability,
                        "Failed to map FlightCapability to StrategicAirframeType.");
            }
        }
        ...
        {
        return (JObject)JObject.FromObject(feature)["geometry"];
        }

        private static IEnumerable<CreateFlightPartRequest> CreateFlightPartsFromWaypoints(
            IList<FlightPlanWaypoint> waypoints, Duration duration)
        {
            ...
            for (var pos = 1; pos < waypoints.Count; pos++)
            {
                ...
                var createFlightPartRequest =
                    CreateFlightPartRequest(pos, highestAltitude, startInstant, endInstant, geography);
        ...
        private static IGeometryObject ConvertWaypointsToLineString(FlightPlanWaypoint startWayPoint,
            FlightPlanWaypoint endWayPoint)
        {
