...
namespace AltitudeAngelWings.Extra.OutboundNotifsWebSocket
{
    public class AaClientWebSocket : AaWebSocket
    {
        ...
        public async Task OpenAsync(Uri uri, Dictionary<string, string> requestHeaders = null)
        {
            ...
            if (ClientSocket.State != WebSocketState.Open) throw new Exception("Failed to open connection");
        ...
        {
            if (ClientSocket == null) throw new InvalidOperationException("Socket not open");
        ...
        public void SetSocketHeaders(Dictionary<string, string> headers)
        {
            ...
            foreach (var header in headers) ClientSocket.Options.SetRequestHeader(header.Key, header.Value);
        }
    }
}