...
namespace AltitudeAngelWings
{
    public static class FeaturePropertiesExtensions
    {
        public static ColorInfo ToColorInfo(this FeatureProperties properties, float opacityAdjust = 1f)
        {
            ...
            if (!int.TryParse(properties.StrokeWidth, out var strokeWidth)) strokeWidth = 1;
        ...
        private static uint ToARGB(string color, string opacity = null, float opacityAdjust = 1f)
        {
            ...
            else if (color.StartsWith("0x", StringComparison.OrdinalIgnoreCase)) color = color.Substring(2);
            ...
            switch (color.Length)
            {
                case 6:
                {
                    if (!float.TryParse(opacity, out var opacityAmount)) opacityAmount = 1f;
                ...
                case 8 when opacity != null:
                    throw new InvalidOperationException(
                        "Color cannot contain an ARGB value if the opacity is specified.");
                case 8:
                {
                    if (!byte.TryParse(color.Substring(0, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture,
                            out opacityByte)) opacityByte = 0xFF;
                ...
                default:
                    throw new ArgumentException(nameof(color),
                        "Color must be either #AARRGGBB, #RRGGBB, 0xAARRGGBB or 0xRRGGBB");
            }
            ...
            for (var i = 0; i < 6; i += 2)
            {
                byte.TryParse(color.Substring(i, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture,
                    out var colorByte);
