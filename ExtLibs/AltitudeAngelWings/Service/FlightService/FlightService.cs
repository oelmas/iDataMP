...
namespace AltitudeAngelWings.Service.FlightService
{
    public class FlightService : IFlightService
    {
            ...
            IOutboundNotifsService notificationsService)
        {
            ...
            if (_settings.SurveillanceMode)
            {
                _disposer.Add(flightDataService.FlightArmed
                    .SubscribeWithAsync(async (i, ct) =>
                        await StartSurveillanceFlight(await _missionPlanner.GetFlightPlan())));
            }
            else
            {
                _disposer.Add(flightDataService.FlightArmed
                    .SubscribeWithAsync(async (i, ct) =>
                        await StartTelemetryFlight(await _missionPlanner.GetFlightPlan())));
        ...
        private async Task StartTelemetryFlight(FlightPlan flightPlan)
        {
            ...
            try
            {
                ...
                var notificationSettings =
                    (WebsocketNotificationProtocolConfiguration)
                    tacticalSettings.Properties.NotificationProtocols.First();
                ...
                await _messagesService.AddMessageAsync(
                    new Message($"Flight {startFlightResponse.Id} approved and underway.")
                        { TimeToLive = TimeSpan.FromSeconds(10) });
        ...
        {
            if (string.IsNullOrEmpty(_settings.CurrentFlightId) ||
                string.IsNullOrEmpty(_settings.CurrentFlightReportId)) return;
            try
            {
                ...
                await _messagesService.AddMessageAsync(
                    new Message($"Flight plan {_settings.CurrentFlightReportId} completed.")
                        { TimeToLive = TimeSpan.FromSeconds(10) });
            }
            catch (Exception ex)
            {
                await _messagesService.AddMessageAsync(new Message(
                    $"ERROR: Failed to complete flight {_settings.CurrentFlightId} and plan {_settings.CurrentFlightReportId}:, {ex}"));
            }
            finally
            {
        ...
        {
            if (isDisposing) _disposer?.Dispose();
        }
    }
}