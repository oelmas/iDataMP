...
namespace AltitudeAngelWings.Service.OutboundNotifs
{
    public class OutboundNotifsService : IOutboundNotifsService
    {
        ...
        {
            if (null == _aaClientWebSocket) await SetupAaClientWebSocket();
        }

        public async Task StopWebSocket()
        {
            if (null != _aaClientWebSocket) await TearDownAaClientWebSocket();
        }
        ...
        private async Task OnMessage(byte[] bytes)
        {
            await _messagesService.AddMessageAsync(
                new Message($"INFO: Received notification message of {bytes.Length} bytes."));
            var notification = new NotificationMessage();
            try
            {
                var msg = Encoding.UTF8.GetString(bytes);
                notification = JsonConvert.DeserializeObject<NotificationMessage>(msg);
                if (notification.Acknowledge) await SendAck(_aaClientWebSocket, notification.Id);
            }
            catch (Exception e)
            {
                await _messagesService.AddMessageAsync(
                    new Message($"ERROR: Failed to deserialize and acknowledge notification message. {e}"));
            }

            await _messagesService.AddMessageAsync(
                new Message($"INFO: Processing {notification.Type} notification message."));
            ...
            try
            {
                switch (notification.Type)
                {
                    ...
                    case OutboundNotifsCommands.Loiter:
                        ...
                        await _missionPlanner.CommandDroneToLoiter((float)loiterProps.Latitude,
                            (float)loiterProps.Longitude, (float)loiterProps.Altitude.Meters);
                        break;
                    ...
                    case OutboundNotifsCommands.PermissionUpdate:
                        ...
                        await _messagesService.AddMessageAsync(
                            new Message($"Flight permissions updated: {permissionProperties.PermissionState}")
                    ...
                    case OutboundNotifsCommands.ConflictClearedInformation:
                        var conflictClearedProperties =
                            notification.Properties.ToObject<ConflictClearedNotificationProperties>();
                    ...
                    case OutboundNotifsCommands.Instruction:
                        var instructionProperties =
                            notification.Properties.ToObject<InstructionNotificationProperties>();
                                ...
                                "Instruction"))
                        {
                            ...
                            if (instructionProperties.Instruction.IndexOf("hold",
                                    StringComparison.InvariantCultureIgnoreCase) >= 0)
                            {
                                await _missionPlanner.CommandDroneToLoiter((float)_missionPlannerState.Latitude,
                                    (float)_missionPlannerState.Longitude, _missionPlannerState.Altitude);
                                break;
                            }

                            if (instructionProperties.Instruction.IndexOf("resume",
                                    StringComparison.InvariantCultureIgnoreCase) >= 0)
                            {
                                await _missionPlanner.CommandDroneAllClear();
                                break;
                            ...
                            if (instructionProperties.Instruction.IndexOf("land",
                                    StringComparison.InvariantCultureIgnoreCase) >= 0)
                            {
                                await _missionPlanner.CommandDroneToLand((float)_missionPlannerState.Latitude,
                                    (float)_missionPlannerState.Longitude);
                                break;
                            }

                            if (instructionProperties.Instruction.IndexOf("return",
                                    StringComparison.InvariantCultureIgnoreCase) >=
                                0) await _missionPlanner.CommandDroneToReturnToBase();
                        }
                        else
                        {
                    ...
                    default:
                        await _messagesService.AddMessageAsync(
                            new Message($"WARN: Unknown notification message type '{notification.Type}'."));
                        break;
                }
            }
            catch (Exception e)
            {
                await _messagesService.AddMessageAsync(
                    new Message($"ERROR: Failed to process {notification.Type} notification message. {e}"));
            }
        }

        private Task OnConnected()
        {
            return _messagesService.AddMessageAsync(
                new Message($"INFO: Notifications web socket connected to {_settings.OutboundNotifsEndpointUrl}."));
        }

        private Task OnDisconnected()
        {
            return _messagesService.AddMessageAsync(new Message("WARNING: Notifications web socket disconnected."));
        }

        private Task OnError(Exception e)
        {
            return _messagesService.AddMessageAsync(new Message($"ERROR: Notifications web socket error. {e}"));
        }
        ...
        {
            await _messagesService.AddMessageAsync(new Message(
                $"INFO: Sending notification acknowledgement: {JsonConvert.SerializeObject(new CommandAcknowledgement { Id = id })}"));
