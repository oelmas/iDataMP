        ...
        public static bool TryReadReport(byte[] bytes, out UavPositionReport report)
        {
            ...
            using (var stream = new MemoryStream(bytes))
            {
                using (var reader = new BinaryReader(stream))
                {
                    ...
                    var numStatuses = reader.ReadByte();

                    var expectedBytes = numStatuses * Marshal.SizeOf(typeof(AutpStatusMessage));
                    if (reader.BytesRemaining() < expectedBytes)
                    {
                        Debug.WriteLine(
                            $"Invalid status message bytes remaining. Expected {expectedBytes} bytes, got {reader.BytesRemaining()}",
                            "Serialization Error");
                        return false;
                    }

                    var statuses = new List<AutpStatusMessage>();
                    for (byte i = 0; i < numStatuses; i++) statuses.Add(reader.ReadStruct<AutpStatusMessage>());
            ...
            using (var stream = new MemoryStream())
            {
                using (var writer = new BinaryWriter(stream))
                {
                    try
                    {
                        ...
                        foreach (var status in item.UavStatus) writer.WriteStruct(status.ToAutpStatusMessage());
                    }
