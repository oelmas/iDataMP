    ...
    public class Hkdf
    {
        ...
        private HMAC GetHmac(byte[] key)
        {
            switch (hashType)
            {
                ...
                default: throw new ArgumentException(nameof(hashType));
            }
        }
        ...
        {
            using (var hmac = GetHmac(salt))
            {
                var privateKey = hmac.ComputeHash(secret);
                var result = Expand(hmac, privateKey, outputLength);
                return result;
            }
        }
        ...
        private byte[] Expand(HMAC hmac, byte[] key, int outputLength)
        {
            ...
            {
                var currentInfo = new byte[resultBlock.Length + 1];
