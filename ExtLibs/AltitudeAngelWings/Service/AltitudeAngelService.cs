...
namespace AltitudeAngelWings.Service
{
    public class AltitudeAngelService : IAltitudeAngelService
    {
        ...
        {
            if (!_settings.CheckEnableAltitudeAngel) return;
            try
            {
        ...
        public void ProcessAllFromCache(IMap map, bool resetFilters = false)
        {
            ...
            if (!IsSignedIn) MapFeatureCache.Clear();
            ...
            if (resetFilters) MapFeatureCache.Values.UpdateFilterInfo(FilterInfoDisplay, resetFilters);
