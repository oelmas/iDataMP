...
namespace AltitudeAngelWings.Service
{
    public class Settings : ISettings
    {
        ...
        public Settings(Func<string, string> loadSetting, Action<string> clearSetting,
        ...
        public string ClientId =>
            OverrideClientUrlSettings ? OverrideClientId : ConfigurationManager.AppSettings["ClientId"];

        public string ClientSecret => OverrideClientUrlSettings
        ...
        public string UrlDomainSuffix => OverrideClientUrlSettings
            ? OverrideUrlDomainSuffix
            : ConfigurationManager.AppSettings["UrlDomainSuffix"];
        ...
        public bool DisableTelemetrySending =>
            Convert.ToBoolean(ConfigurationManager.AppSettings["DisableTelemetrySending"]);
        ...
        public SymmetricEncryptionType EncryptionType =>
            SymmetricEncryptionType.Aes128; //TODO add to config file to hide from mission planner
        ...
        {
            get => Get(nameof(MinimumPollInterval), TimeSpan.FromMilliseconds(500),
                s => TimeSpan.FromMilliseconds(double.Parse(s)));
            set => Set(nameof(MinimumPollInterval), value);
        }

        public IList<FilterInfoDisplay> MapFilters
        {
            get => Get(nameof(MapFilters), new List<FilterInfoDisplay>(),
                JsonConvert.DeserializeObject<List<FilterInfoDisplay>>);
        ...
        private void Set<T>(string settingName, T value, Func<T, string> setSetting = null)
        {
            ...
            if (setSetting == null) setSetting = v => v.ToString();
