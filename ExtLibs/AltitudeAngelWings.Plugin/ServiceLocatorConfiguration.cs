    ...
    public class ServiceLocatorConfiguration : IServiceLocatorConfiguration
    {
        public void Configure()
        {
            ...
            ServiceLocator.Register<IUiThreadInvoke>(l => new UiThreadInvoke(
                action => Task.Factory.FromAsync(l.Resolve<PluginHost>().MainForm.BeginInvoke(action),
                    result => l.Resolve<PluginHost>().MainForm.EndInvoke(result))));
        ...
        {
            if (flightPlanner == null) return new List<Locationwp>();
            var getFlightPlanLocations = flightPlanner.GetType()
                .GetMethod("GetFlightPlanLocations", BindingFlags.Instance | BindingFlags.NonPublic);
