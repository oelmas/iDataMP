...
namespace AltitudeAngelWings.Plugin
{
    internal partial class AASettings : Form
    {
            ...
            IMissionPlanner missionPlanner)
        {
            ...
            txt_ExistingFlightPlanId.Text = _settings.ExistingFlightPlanId == Guid.Empty
                ? ""
                : _settings.ExistingFlightPlanId.ToString();
        ...
        private void AddUpdateFilterInfoTree(TreeNode parent)
        {
                         ...
                         .Where(i => i.ParentName == parent?.Name))
            {
                ...
                node.Tag = item;
                if (node.Checked != item.Visible) node.Checked = item.Visible;
        ...
        private void RefreshControlStates(bool signOut = false, bool processMap = false, bool resetFilters = false)
        {
            ...
            if (signOut) _altitudeAngelService.DisconnectAsync();

            if (processMap) ProcessMapsFromCache(resetFilters);
            ...
            lbl_FlightReportName.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                           !chk_UseExistingFlightPlanId.Checked;
            txt_FlightReportName.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                           !chk_UseExistingFlightPlanId.Checked;
            lbl_FlightReportDescription.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                                  !chk_UseExistingFlightPlanId.Checked;
            txt_FlightReportDescription.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                                  !chk_UseExistingFlightPlanId.Checked;
            lbl_FlightReportDuration.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                               !chk_UseExistingFlightPlanId.Checked;
            txt_FlightReportDuration.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                               !chk_UseExistingFlightPlanId.Checked;
            chk_FlightReportCommercial.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                                 !chk_UseExistingFlightPlanId.Checked;
            chk_FlightReportLocalScope.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                                 !chk_UseExistingFlightPlanId.Checked;
            ...
            chk_AllowSms.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                   !chk_UseExistingFlightPlanId.Checked;
            txt_ContactPhone.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                       !chk_UseExistingFlightPlanId.Checked;
            chk_IcaoAddress.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                      !chk_UseExistingFlightPlanId.Checked;
            txt_IcaoAddress.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                      !chk_UseExistingFlightPlanId.Checked && chk_IcaoAddress.Checked;
            chk_SerialNumber.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                       !chk_UseExistingFlightPlanId.Checked;
            txt_SerialNumber.Enabled = _altitudeAngelService.IsSignedIn && chk_FlightReportEnable.Checked &&
                                       !chk_UseExistingFlightPlanId.Checked && chk_SerialNumber.Checked;
            ...
            SetTabVisibility(tabPageFlight,
                _settings.CheckEnableAltitudeAngel && _altitudeAngelService.IsSignedIn &&
                chk_FlightReportEnable.Checked);
            ResumeLayout(true);
        }
        ...
        private void SetTabVisibility(TabPage tabPage, bool visible)
        {
            ...
            if (visible && !exists) tabPages.TabPages.Insert(tabPages.TabPages.IndexOf(tabPageAbout), tabPage);
            if (!visible && exists) tabPages.TabPages.Remove(tabPage);
        }
    }
}