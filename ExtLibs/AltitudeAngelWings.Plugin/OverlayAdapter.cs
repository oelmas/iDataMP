...
namespace AltitudeAngelWings.Plugin
{
    internal class OverlayAdapter : IOverlay
    {
        ...
        public bool IsVisible
        {
            ...
            {
                var value = false;
                _context.Send(state => { value = _overlay.IsVisibile; }, null);
                return value;
            }
            set { _context.Send(state => { _overlay.IsVisibile = value; }, null); }
        }
        ...
        public void RemovePolygonsExcept(List<string> names)
        {
            _context.Send(_ =>
            {
                var remove = _overlay.Polygons
                    ...
                    .ToArray();
                foreach (var polygon in remove) _overlay.Polygons.Remove(polygon);
            }, null);
        }
        ...
        public void RemoveLinesExcept(List<string> names)
        {
            _context.Send(_ =>
            {
                var remove = _overlay.Routes
                    .Where(p => !names.Contains(p.Name))
                    .ToArray();
                foreach (var route in remove) _overlay.Routes.Remove(route);
            }, null);
        }
