...
namespace MissionPlanner
{
    public class CurrentState : ICloneable, IDisposable
    {
        ...
        private DateTime _messageHighTime;
        ...
        [GroupText("Position")]
        public double verticalspeed_fpm => vz * -3.28084 * 60;
        ...
        [GroupText("Position")]
        public float turng => (float)(1 / Math.Cos(MathHelper.deg2rad * roll));
        ...
        [GroupText("NAV")] public float targetalt { get; private set; }
        ...
        /// </summary>
        public string message => messages.LastOrDefault().message;
        ...
        public string messageHigh
        {
            get
            {
                if (DateTime.Now > _messageHighTime.AddSeconds(10)) return "";
                return _messagehigh.TrimUnPrintable();
            }
            set
            {
                if (value == null || value == "")
        ...
        [GroupText("Position")] public PointLatLngAlt HomeLocation { get; set; } = new PointLatLngAlt();
                ...
                try
                {
                    ...
                    foreach (var sublist in list)
                    {
                        ...
                        if (PolygonTools.isInside(
                                sublist.CloseLoop()
                                    .Select(a => new PolygonTools.Point(a.Value.y / 1e7, a.Value.x / 1e7)).ToList(),
        ...
        [GroupText("Software")] public float freemem { get; set; }
        ...
        [GroupText("Other")] public double timesincelastshot { get; set; }
        ...
        [GroupText("Enviromental")] public int KIndex => KIndexstatic;
        ...
        [GroupText("Generator")] public float gen_status { get; set; }

        [GroupText("Generator")] public float gen_speed { get; set; }

        [GroupText("Generator")] public float gen_current { get; set; }

        [GroupText("Generator")] public float gen_voltage { get; set; }

        [GroupText("Generator")] public uint gen_runtime { get; set; }

        [GroupText("Generator")] public int gen_maint_time { get; set; }
        ...
        private void Parent_OnPacketReceived(object sender, MAVLink.MAVLinkMessage mavLinkMessage)
        {
            if ((mavLinkMessage.sysid == parent.sysid && mavLinkMessage.compid == parent.compid)
                || mavLinkMessage.msgid ==
                (uint)MAVLink.MAVLINK_MSG_ID
                    .RADIO // propagate the RADIO/RADIO_STATUS message across all devices on this link
                ...
                switch (mavLinkMessage.msgid)
                {
...

                    {
                        ...
                        if (ekfvelv >= 1) messageHigh = Strings.ERROR + " " + Strings.velocity_variance;

                        if (ekfcompv >= 1) messageHigh = Strings.ERROR + " " + Strings.compass_variance;

                        if (ekfposhor >= 1) messageHigh = Strings.ERROR + " " + Strings.pos_horiz_variance;

                        if (ekfposvert >= 1) messageHigh = Strings.ERROR + " " + Strings.pos_vert_variance;

                        if (ekfteralt >= 1) messageHigh = Strings.ERROR + " " + Strings.terrain_alt_variance;
...

                    {
                        ...
                        else
                        {
                            ...
                            if (armed == false && newarmed) timeSinceArmInAir = 0;
...

                    {
                                 ...
                                 sensors_present.rc_receiver)
                        {
                            ...
                            if (reporterror) messageHigh = Strings.NORCReceiver;
                        }
                        ...
                        else if (!sensors_health.differential_pressure && sensors_enabled.differential_pressure &&
                                 sensors_present.differential_pressure)
                        {
...

                    {
                        ...
                        var temp_battery_voltage = bats.voltages.Sum(a => a != ushort.MaxValue ? a / 1000.0 : 0) +
                                                   bats.voltages_ext.Sum(a => a != ushort.MaxValue ? a / 1000.0 : 0);
                        ...
                        if (bats.id == 0)
                        {
                            if (bats.voltages[0] != ushort.MaxValue)
                            {
                                ...
                                if (bats.voltages_ext[0] != ushort.MaxValue)
                                    battery_cell11 = bats.voltages_ext[0] / 1000.0;
                                else battery_cell11 = 0.0;
                                if (bats.voltages_ext[1] != ushort.MaxValue)
                                    battery_cell12 = bats.voltages_ext[1] / 1000.0;
                                else battery_cell12 = 0.0;
                                if (bats.voltages_ext[2] != ushort.MaxValue)
                                    battery_cell13 = bats.voltages_ext[2] / 1000.0;
                                else battery_cell13 = 0.0;
                                if (bats.voltages_ext[3] != ushort.MaxValue)
                                    battery_cell14 = bats.voltages_ext[3] / 1000.0;
...

                    {
                        ...
                        var mav_value_name = Encoding.UTF8.GetString(named_float.name);

                        var ind = mav_value_name.IndexOf('\0');
                        if (ind != -1)
                            mav_value_name = mav_value_name.Substring(0, ind);

                        var name = "MAV_" + mav_value_name.ToUpper();

                        var value = named_float.value;
                        ...
                        if (field == null)
                        {
                            short i;
                            for (i = 0; i < 10; i++)
                                if (!custom_field_names.ContainsKey("customfield" + i))
                                    break;
                            if (i < 10)
                            {
                                field = "customfield" + i;
        ...
        public static int StringCompareTo(string a, string b)
        {
            ...
            while (i < arr1.Length && j < arr2.Length)
                ...
                {
                    if (char.ToLower(arr1[i]) > char.ToLower(arr2[j])) return 1;
                    if (char.ToLower(arr1[i]) < char.ToLower(arr2[j])) return -1;
                    i++;
                    j++;
                }
