...
namespace MissionPlanner
{
    ...
    {
        private Dictionary<uint, Dictionary<uint, List<irate>>> _bps =
            new Dictionary<uint, Dictionary<uint, List<irate>>>();

        private Dictionary<uint, Dictionary<uint, T>> _history = new Dictionary<uint, Dictionary<uint, T>>();

        private readonly object _lock = new object();

        private Dictionary<uint, Dictionary<uint, List<irate>>> _rate =
            new Dictionary<uint, Dictionary<uint, List<irate>>>();
        ...
        public IEnumerable<T> this[byte sysid, byte compid]
        {
            get
            {
                ...
                foreach (var msg in toArray(_history[id].Values)) yield return msg;
            }
        }
        ...
        {
            var sysids = new List<byte>();
            foreach (var id in toArray(_history.Keys)) sysids.Add(GetFromID(id).sysid);
        ...
        {
            var compids = new List<byte>();
            foreach (var id in toArray(_history.Keys)) compids.Add(GetFromID(id).compid);
        ...
        public double SeenRate(byte sysid, byte compid, uint msgid)
        {
            ...
            try
            {
                ...
                var msgrate = data.Where(a => { return a.dateTime > start && a.dateTime < end; })
        ...
        public double SeenBps(byte sysid, byte compid, uint msgid)
        {
            ...
            try
            {
                var starttime = data.First().dateTime;
                starttime = starttime < start ? start : starttime;
                var msgbps = data.Where(a => { return a.dateTime > start && a.dateTime < end; })
                    .Sum(a => a.value / (end - starttime).TotalSeconds);
                return msgbps;
            }
            catch
        ...
        private uint GetID(byte sysid, byte compid)
        {
            return sysid * 256u + compid;
        }

        private (byte sysid, byte compid) GetFromID(uint id)
        {
