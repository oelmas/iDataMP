    ...
    public class APFirmware
    {
        ...
        public static void GetList(string url = "https://firmware.ardupilot.org/manifest.json.gz", bool force = false)
        {
            lock (getListlock)
            {
                ...
                var gz = new GZipStream(mssrc, CompressionMode.Decompress);
                gz.CopyTo(msdest);
                msdest.Position = 0;
        ...
        public static long? GetBoardID(DeviceInfo device, bool boardidcheck = true)
        {
            ...
            var ans = Manifest.Firmware.Where(a => a.Platform?.ToLower() == device.board?.ToLower() ||
                                                   a.BootloaderStr.Any(b => b?.ToLower() == device.board?.ToLower()));

            if (boardidcheck)
                ans = ans.Where(a => a.BoardId != 0);

            if (ans.Any()) return ans.First().BoardId;

            if (device.hardwareid == null)
                return null;

            // match the vid/pid
            var vidpid = new Regex("VID_([0-9a-f]+)&PID_([0-9a-f]+)&", RegexOptions.IgnoreCase);
            ...
            if (vidpid.IsMatch(device.hardwareid))
            {
                ...
                var vidandusbdesc = Manifest.Firmware.Where(a =>
                    a.Usbid.Any(b => b.ToLower().Contains(lookfor.ToLower())) && a.BoardId != 0);

                if (vidandusbdesc.Any()) return vidandusbdesc.First().BoardId;
            }

            return null;
        }

        public static List<FirmwareInfo> GetOptions(DeviceInfo device, RELEASE_TYPES? reltype = null,
            MAV_TYPE? mav_type = null)
        {
            GetList();
