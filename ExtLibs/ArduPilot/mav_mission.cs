...
namespace MissionPlanner.ArduPilot
{
    public class mav_mission
    {
        ...
        public static async Task<List<Locationwp>> download(MAVLinkInterface port, byte sysid, byte compid,
            MAVLink.MAV_MISSION_TYPE type, Action<int, string> progress = null)
        {
            var commandlist = new List<Locationwp>();

            try
            {
                if (!port.BaseStream.IsOpen) throw new Exception("Please Connect First!");

                var use_supportfence = (port.MAVlist[sysid, compid].cs.capabilities &
                                        (uint)MAVLink.MAV_PROTOCOL_CAPABILITY.MISSION_FENCE) > 0;

                var use_int = (port.MAVlist[sysid, compid].cs.capabilities &
                               (uint)MAVLink.MAV_PROTOCOL_CAPABILITY.MISSION_INT) > 0;
                ...
                if (!use_supportfence && !port.MAVlist[sysid, compid].mavlinkv2 &&
        ...
        public static async Task upload(MAVLinkInterface port, byte sysid, byte compid, MAVLink.MAV_MISSION_TYPE type,
            List<Locationwp> commandlist, Action<int, string> progress = null)
        {
            try
            {
                if (!port.BaseStream.IsOpen) throw new Exception("Please connect first!");
                ...
                for (a = 0; a < commandlist.Count; a++)
                {
                    ...
                    // try send the wp
                    var ans = await port.setWPAsync(sysid, compid, temp, (ushort)uploadwpno,
                    ...
                    if (ans == MAVLink.MAV_MISSION_RESULT.MAV_MISSION_ERROR)
                    {
                        ...
                        await port.setWPPartialUpdateAsync(sysid, compid, (ushort)uploadwpno, (ushort)commandlist.Count,
                    ...
                    if (ans == MAVLink.MAV_MISSION_RESULT.MAV_MISSION_INVALID_SEQUENCE)
                    {
                        ...
                        {
                            // resend for partial upload
                            await port.setWPPartialUpdateAsync(sysid, compid, (ushort)uploadwpno,
                                (ushort)commandlist.Count, type).ConfigureAwait(false);
                            // reupload this point.
                        }
                    ...
                    if (ans != MAVLink.MAV_MISSION_RESULT.MAV_MISSION_ACCEPTED)
                        throw new Exception("Upload " + type + " failed " +
        ...
        public static async Task uploadPartial(MAVLinkInterface port, byte sysid, byte compid,
            MAVLink.MAV_MISSION_TYPE type, List<Locationwp> commandlist, ushort start,
