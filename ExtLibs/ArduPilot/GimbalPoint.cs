...
namespace MissionPlanner.Utilities
{
    public class GimbalPoint
    {
        ...
        private static float angle_input(bool rev, float radio_in, float radio_min, float radio_max, float angle_min,
            float angle_max)
        {
        ...
        private static int channelpwm(int channel)
        {
            if (channel == 1)
                return (int)comPort.MAV.cs.ch1out;
            if (channel == 2)
                return (int)comPort.MAV.cs.ch2out;
            if (channel == 3)
                return (int)comPort.MAV.cs.ch3out;
            if (channel == 4)
                return (int)comPort.MAV.cs.ch4out;
            if (channel == 5)
                return (int)comPort.MAV.cs.ch5out;
            if (channel == 6)
                return (int)comPort.MAV.cs.ch6out;
            if (channel == 7)
                return (int)comPort.MAV.cs.ch7out;
            if (channel == 8)
                return (int)comPort.MAV.cs.ch8out;
        ...
        public static double ConvertPwmtoAngle(axis axis)
        {
            ...
            switch (axis)
            {
                case axis.roll:
                    ...
                    var minr = (float)comPort.MAV.param["RC" + rollchannel + "_MIN"];
                    var maxr = (float)comPort.MAV.param["RC" + rollchannel + "_MAX"];
                    var minroll = (float)comPort.MAV.param["MNT_ANGMIN_ROL"];
                    var maxroll = (float)comPort.MAV.param["MNT_ANGMAX_ROL"];
                    var revr = comPort.MAV.param.ContainsKey("RC" + rollchannel + "_REV")
                ...
                case axis.pitch:
                    ...
                    var minp = (float)comPort.MAV.param["RC" + pitchchannel + "_MIN"];
                    var maxp = (float)comPort.MAV.param["RC" + pitchchannel + "_MAX"];
                    var minpitch = (float)comPort.MAV.param["MNT_ANGMIN_TIL"];
                    var maxpitch = (float)comPort.MAV.param["MNT_ANGMAX_TIL"];
                    var revp = comPort.MAV.param.ContainsKey("RC" + pitchchannel + "_REV")
                ...
                case axis.yaw:
                    ...
                    var miny = (float)comPort.MAV.param["RC" + yawchannel + "_MIN"];
                    var maxy = (float)comPort.MAV.param["RC" + yawchannel + "_MAX"];
                    var minyaw = (float)comPort.MAV.param["MNT_ANGMIN_PAN"];
                    var maxyaw = (float)comPort.MAV.param["MNT_ANGMAX_PAN"];
                    var revy = comPort.MAV.param.ContainsKey("RC" + yawchannel + "_REV")
        ...
        public static PointLatLngAlt ProjectPoint(MAVLinkInterface comPort)
        {
            ...
            var currentlocation = new PointLatLngAlt(comPort.MAV.cs.lat, comPort.MAV.cs.lng);
            ...
            var distout = 10;
            var newpos = PointLatLngAlt.Zero;
            ...
            while (distout < 1000)
            {
                ...
                var newposdist = currentlocation.newpos(yawangle + comPort.MAV.cs.yaw, distout);
                newposdist.Alt = srtm.getAltitude(newposdist.Lat, newposdist.Lng).alt;

                // get another point 50 infront
                var newposdist2 = currentlocation.newpos(yawangle + comPort.MAV.cs.yaw, distout + 50);
                newposdist2.Alt = srtm.getAltitude(newposdist2.Lat, newposdist2.Lng).alt;

                // get the flat terrain distance out - at 0 alt
                var distflat = Math.Tan((90 + pitchangle) * MathHelper.deg2rad) * comPort.MAV.cs.altasl;

                // x is dist from plane, y is alt
                var newpoint = FindLineIntersection(new Point(0, comPort.MAV.cs.altasl),
                    new Point((float)distflat),
                    new Point(distout, (float)newposdist.Alt),
