 ...
 * IOCTL commands for joystick driver
                    ...
                    try
                    {
                        ...
                        */
                        state = new LinuxJoystickState(
                            j.Axis.ToDictionary(a => a.Key, a => (ushort)(a.Value + 65535 / 2)),
        ...
        public override int getNumButtons()
        {
            ...
            var obj1Handle = GCHandle.Alloc(number_of_buttons, GCHandleType.Pinned);
        ...
        private uint JSIOCGNAME(uint len)
        {
            return _IOC(_IOC_READ, 'j', 0x13, len);
            /* get identifier string */
        }

        private static uint _IOC_TYPECHECK(Type t)
        {
            return (uint)Marshal.SizeOf(t);
        }

        private static uint _IOC(uint dir, uint type, uint nr, uint size)
        {
        ...
        private static uint _IOR(uint type, uint nr, Type size)
        {
        ...
        public struct js_event
        {
            ...
            /// </summary>
            public uint time => BitConverter.ToUInt32(raw, 0); /* event timestamp in milliseconds */

            /// <summary>
            ///     4 value
            /// </summary>
            public short value => BitConverter.ToInt16(raw, 4); /* value */

            /// <summary>
            ///     6 event type
            /// </summary>
            public byte type => raw[6]; /* event type */

            /// <summary>
            ///     7 axis/button number
            /// </summary>
            public byte number => raw[7]; /* axis/button number */
        ...
        public class Joystick
        {
            ...
            public void DetectChange(js_event buff)
            {
                ...
                if (checkBit(buff.type, (byte)TYPE.AXIS))
                {
                    ...
                    Axis[buff.number] = value;
                    return;
                }

                if (checkBit(buff.type, (byte)TYPE.BUTTON))
                {
                    Button[buff.number] = buff.value == (byte)STATE.PRESSED;
                }
            }

            private bool checkBit(byte value, byte flag)
            {
                var c = (byte)(value & flag);
                return c == flag;
            }
