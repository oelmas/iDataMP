...
namespace MissionPlanner.ArduPilot
{
    public static class parampck
    {
        ...
        private static readonly int magic = 0x671b;

        private static readonly int magic_with_defaults = 0x671c;
        ...
        */
        private static readonly Dictionary<int, (int type_len, char type_format)> map =
            new Dictionary<int, (int type_len, char type_format)>
            {
                { 1, (1, 'b') },
                { 2, (2, 'h') },
                { 3, (4, 'i') },
        ...
        public static MAVLink.MAVLinkParamList unpack(byte[] data)
        {
            ...
            var with_defaults = magic2 == magic_with_defaults;
            ...
            while (true)
            {
                ...
                if (!map.TryGetValue(ptype, out var mapped))
                    return null;
                ...
                for (var i = dataPointer; i < dataPointer + name_len; i++)
                ...
                list.Add(new MAVLink.MAVLinkParam(name, vdata,
                    ...
                    ptype == 4 ? MAVLink.MAV_PARAM_TYPE.REAL32 : 0,
                    ptype == 1 ? MAVLink.MAV_PARAM_TYPE.INT8 :
                    ptype == 2 ? MAVLink.MAV_PARAM_TYPE.INT16 :
                    ptype == 3 ? MAVLink.MAV_PARAM_TYPE.INT32 :
                    ptype == 4 ? MAVLink.MAV_PARAM_TYPE.REAL32 : 0,
