...
namespace MissionPlanner
{
    public class MAVLinkInterface : MAVLink, IDisposable, IMAVLinkInterface, IMAVLinkInterfaceLogRead
    {
        ...
        private readonly
            List<(MAVLINK_MSG_ID msgId, Func<MAVLinkMessage, bool> function, bool exclusive, byte sysid, byte compid)>
            Subscriptions =
        ...
        private int _compidcurrent;

        private volatile bool _giveComport;
        ...
        private int _mavlink1count;
        private int _mavlink2count;
        private int _mavlink2signed;
        ...
        private int _parampoll;

        private int _sysidcurrent;
        ...
        private uint request_id;

        private readonly Dictionary<Stream, Tuple<string, long>> streamfncache =
            new Dictionary<Stream, Tuple<string, long>>();

        private readonly double t7 = 1.0e7;
        ...
        public MAVLinkInterface()
        {
            ...
            packetcount = 0;
            _bytesReceivedSubj = new Subject<int>();
            _bytesSentSubj = new Subject<int>();
            WhenPacketLost = new Subject<int>();
            WhenPacketReceived = new Subject<int>();
            ...
            _logplaybackfile = null;
            logfile = null;
            rawlogfile = null;
            _bps1 = 0;
            _bps2 = 0;
            _bpstime = DateTime.MinValue;
            _mavlink1count = 0;
            _mavlink2count = 0;
            _mavlink2signed = 0;
        ...
        public ICommsSerial BaseStream
        {
            ...
            set
            {
                ...
                if (_baseStream != null)
                {
                    try
                    {
                        if (_baseStream.IsOpen) _baseStream.Close();
                    }
                    catch
                    {
                    }

                    IDisposable dsp = _baseStream;
                    if (dsp != null)
                        try
                        {
                            dsp.Dispose();
        ...
        public MAVState MAV
        {
            get => MAVlist[sysidcurrent, compidcurrent];
        ...
        public event EventHandler<MAVLinkMessage> OnPacketReceived
        {
            add
            {
                log.Debug("Subscribed " + new StackTrace(1, true)?.GetFrame(0) + " " + value?.Method?.Name +
            ...
            remove
            {
                log.Debug("UnSubscribed " + new StackTrace(1, true)?.GetFrame(0) + " " +
        ...
        public event EventHandler<MAVLinkMessage> OnPacketSent
        {
            ...
            remove
            {
                log.Debug("UnSubscribed " + new StackTrace(1, true)?.GetFrame(0) + " " + value.Method.Name +
                          " " + value.Target.GetType().Name +
                          " " + value);
                _OnPacketSent -= value;
            }
        ...
        public event EventHandler MavChanged
        {
            add
            {
                log.Info("Subscribed " + new StackTrace(1, true)?.GetFrame(0) + " " + value.Method.Name +
                         " " + value.Target.GetType().Name +
                         " " + value);
                _MavChanged += value;
            }
            remove
            {
                log.Info("UnSubscribed " + new StackTrace(1, true)?.GetFrame(0) + " " + value.Method.Name +
                         " " + value.Target.GetType().Name +
                         " " + value);
                _MavChanged -= value;
            }
        ...
        public void sendPacket(object indata, int sysid, int compid)
        {
            ...
            if (!validPacket) log.Info("Mavlink : NOT VALID PACKET sendPacket() " + indata.GetType());
        }
        ...
        {
            // check for a camera
            if (tuple.Item2 >= (byte)MAV_COMPONENT.MAV_COMP_ID_CAMERA &&
                tuple.Item2 <= (byte)MAV_COMPONENT.MAV_COMP_ID_CAMERA6)
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    {
                        // Open holds this
                        while (!_openComplete) await Task.Delay(1000);
            ...
            // gimbals
            if (tuple.Item2 >= (byte)MAV_COMPONENT.MAV_COMP_ID_GIMBAL &&
        ...
        public void Open(bool getparams, bool skipconnectedcheck = false, bool showui = true)
        {
            ...
            if (_ParamListChanged != null) _ParamListChanged(this, null);
        }
        ...
        private void OpenBg(IProgressReporterDialogue PRsender, bool getparams)
        {
            ...
            try
            {
                ...
                var hbhistory = new List<MAVLinkMessage>();

                var start = DateTime.Now;
                var deadline = start.AddSeconds(CONNECT_TIMEOUT_SECONDS);

                var countDown = new Timer { Interval = 1000, AutoReset = false };
                countDown.Elapsed += (sender, e) =>
                {
                    var secondsRemaining = (deadline - e.SignalTime).Seconds;
...
Mission Planner waits for 2 valid heartbeat packets before connecting");
                    ...
                    if (buffer.Length > 0)
                    {
                        var hb = buffer.ToStructure<mavlink_heartbeat_t>();

                        // no GCS's and no broadcast compid's (ping adsb)
                        if (hb.type != (byte)MAV_TYPE.GCS && buffer.compid != 0) hbhistory.Add(buffer);
                    }
                ...
                var temp = ("Mission Planner " + getAppVersion() + "\0").ToCharArray();
        ...
        public byte device_op(byte sysid, byte compid, out byte[] outputbuffer, DEVICE_OP_BUSTYPE bustype,
            ...
            byte address, byte regstart, byte count, byte[] writebytes = null)
        {
            ...
            {
                var read = new mavlink_device_op_read_t
                {
                    target_system = sysid,
                    target_component = compid,
                    request_id = request_id++,
            ...
            var start = DateTime.Now;
            while (!responce && start.AddSeconds(1) > DateTime.Now) Thread.Sleep(1);
        ...
        public async Task<MAVLinkMessage> getHeartBeatAsync()
        {
            ...
            sendPacket(new mavlink_heartbeat_t
            {
            ...
            while (true)
            {
                ...
                if (buffer.Length > 5)
                {
                    ...
                    {
                        var hl = buffer.ToStructure<mavlink_high_latency2_t>();
        ...
        public void generatePacket(int messageType, object indata, int sysid, int compid, bool forcemavlink2 = false,
            bool forcesigning = false)
        {
            if (BaseStream == null || !BaseStream.IsOpen) return;
            ...
            lock (objlock)
            {
                ...
                else
                {
                    ...
                    var checksum = MavlinkCRC.crc_calculate(packet, packet[1] + MAVLINK_NUM_HEADER_BYTES);
                    ...
                    lock (logfile)
                    {
                        var datearray =
                            BitConverter.GetBytes(
                                (ulong)((DateTime.UtcNow - new DateTime(1970, 1, 1)).TotalMilliseconds * 1000));
        ...
        public bool setupSigning(byte sysid, byte compid, string userseed, byte[] key = null)
        {
            ...
            var sign = new mavlink_setup_signing_t();
            if (!clearkey)
            {
                sign.initial_timestamp =
                    (ulong)((DateTime.UtcNow - new DateTime(2015, 1, 1)).TotalMilliseconds * 100);
            ...
            sign.target_component = compid;
            sign.target_system = sysid;
            ...
            bool force = false)
        {
            ...
            var temp = paramname.ToCharArray();
            ...
            if ((MAVlist[sysid, compid].cs.capabilities & (uint)MAV_PROTOCOL_CAPABILITY.PARAM_FLOAT) > 0 ||
                ...
                req.param_value = new MAVLinkParam(paramname, value,
                    MAVlist[sysid, compid].param_types[paramname]).float_value;

            var currentparamcount = MAVlist[sysid, compid].param.Count;
            ...
            var sub1 = SubscribeToPacketType(MAVLINK_MSG_ID.PARAM_VALUE, buffer =>
            {
                ...
                {
                    var par = buffer.ToStructure<mavlink_param_value_t>();

                    var st = Encoding.UTF8.GetString(par.param_id);
        ...
        public async Task<MAVLinkParamList> getParamListMavftpAsync(byte sysid, byte compid)
        {
            var sub2 = SubscribeToPacketType(MAVLINK_MSG_ID.STATUSTEXT, buffer =>
            {
                if (buffer.msgid == (byte)MAVLINK_MSG_ID.STATUSTEXT)
                {
                    ...
                    var logdata = Encoding.UTF8.GetString(msg.text);

                    var ind = logdata.IndexOf('\0');
                    if (ind != -1)
                    ...
                    else if (logdata.ToLower().Contains("frame")) MAVlist[sysid, compid].FrameString = logdata;
                }
        ...
        {
            var indexsreceived = new List<int>();

            // create new list so if canceled we use the old list
            var newparamlist = new MAVLinkParamList();
            ...
            var retry = 0;
            var onebyone = false;
            var lastonebyone = DateTime.MinValue;
            ...
            var sub1 = SubscribeToPacketType(MAVLINK_MSG_ID.PARAM_VALUE, buffer =>
            {
                    ...
                    buffer.compid == req.target_component)
                {
                    ...
                    var paramID = Encoding.UTF8.GetString(par.param_id);

                    var pos = paramID.IndexOf('\0');
                    ...
                    if (frmProgressReporter != null)
                        frmProgressReporter.UpdateProgressAndStatus(indexsreceived.Count * 100 / param_total,
            ...
            do
            {
                        ...
                        for (short i = 0; i <= param_total - 1; i++)
                            if (!indexsreceived.Contains(i))
                            {
                                ...
                                try
                                {
                                    ...
                                    var req2 = new mavlink_param_request_read_t
                                    {
                                        target_system = sysid,
                                        target_component = compid,
                                        param_index = i,
                ...
                if (logreadmode && logplaybackfile.BaseStream.Position >= logplaybackfile.BaseStream.Length) break;
        ...
        public void getParamPoll()
        {
            ...
            if (MAV.param.TotalReceived >= MAV.param.TotalReported) return;

            // if we are connected as primary to a vehicle where we dont have all the params, poll for them
            var i = (short)(_parampoll % MAV.param.TotalReported);
            ...
            bool requireresponce = true)
        {
            ...
            if (index == -1) req.param_id = name.MakeBytes();
        ...
        {
            var req = new mavlink_request_data_stream_t
            {
        ...
        [Obsolete]
        public void setWPACK(MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
            setWPACK(MAV.sysid, MAV.compid, type);
        }

        public void setWPACK(byte sysid, byte compid, MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
            var req = new mavlink_mission_ack_t
            {
                target_system = sysid,
                target_component = compid,
                type = 0,
        ...
        public async Task<bool> setWPCurrentAsync(byte sysid, byte compid, ushort index)
        {
            ...
            var req = new mavlink_mission_set_current_t
            {
                target_system = sysid,
                target_component = compid,
                seq = index
        ...
        public bool doMotorTest(int motor, MOTOR_TEST_THROTTLE_TYPE thr_type, int throttle, int timeout,
            int motorcount = 0)
        {
            return doCommand(MAV_CMD.DO_MOTOR_TEST, motor, (byte)thr_type,
                throttle, timeout, motorcount, 0, 0);
        }
            ...
            float p5, float p6, float p7, bool requireack = true, Action uicallback = null)
        {
            ...
            if (actionid == MAV_CMD.PREFLIGHT_CALIBRATION && p6 == 1)
            {
                // compassmot
            ...
            MAV_FRAME frame = MAV_FRAME.GLOBAL)
        {
            ...
            var req = new mavlink_command_int_t
            {
                target_system = sysid,
                target_component = compid,
        ...
        public void SendAck()
        {
            var ack = new mavlink_command_ack_t();
        ...
        public void SendSerialControl(SERIAL_CONTROL_DEV port, ushort timeoutms, byte[] data, uint baudrate = 0,
            bool close = false)
        {
            var ctl = new mavlink_serial_control_t
            {
                baudrate = baudrate,
                device = (byte)port,
                timeout = timeoutms,
        ...
        public void SendRCOverride(byte sysid, byte compid, ushort rc1, ushort rc2, ushort rc3, ushort rc4, ushort rc5,
            ushort rc6, ushort rc7, ushort rc8)
        {
            var rc = new mavlink_rc_channels_override_t
            {
                target_component = compid,
                target_system = sysid,
                chan1_raw = rc1,
        ...
        public void SendManualControl(byte sysid, byte compid, short x, short y, short z, short r, ushort buttons)
        {
            var mc = new mavlink_manual_control_t
            {
                target = sysid,
                x = x,
                y = y,
        ...
        public void requestDatastream(MAV_DATA_STREAM id, int hzrate, int sysid = -1, int compid = -1)
        {
            ...
            switch (id)
            {
                ...
                case MAV_DATA_STREAM.EXTENDED_STATUS:
                    ...
                    if (hzratecheck(pps, hzrate)) return;
        ...
        [Obsolete]
        public ushort getWPCount(MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
            return getWPCountAsync(MAV.sysid, MAV.compid, type).AwaitSync();
        }

        public ushort getWPCount(byte sysid, byte compid, MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
        ...
        [Obsolete]
        public Locationwp getWP(ushort index, MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
            ...
            MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
            ...
            var use_int = (MAVlist[sysid, compid].cs.capabilities & (uint)MAV_PROTOCOL_CAPABILITY.MISSION_INT) > 0;

            object req;

            if (use_int)
            {
                var reqi = new mavlink_mission_request_int_t
                {
                    target_system = sysid,
                    target_component = compid,
                    mission_type = (byte)type,
            ...
            {
                var reqf = new mavlink_mission_request_t
                {
                    target_system = sysid,
                    target_component = compid,
                    mission_type = (byte)type,
            ...
            while (true)
            {
                ...
                if (buffer.Length > 5)
                {
                    ...
                    if (buffer.msgid == (byte)MAVLINK_MSG_ID.MISSION_ITEM_INT && buffer.sysid == sysid &&
                        buffer.compid == compid)
                    {
                        //Console.WriteLine("getwp ans " + DateTime.Now.Millisecond);
                ...
                if (datin.Length > 5)
                {
...

                    {
                        ...
                        foreach (var field in test.GetFields())
                        {
                            ...
                            var fieldValue = field.GetValue(data); // Get value
                            ...
                            if (field.FieldType.IsArray)
                            {
                                ...
                                if (fieldValue.GetType() == typeof(byte[]))
                                    try
                                    {
                                        var crap = (byte[])fieldValue;

                                        foreach (var fiel in crap)
                                            if (fiel == 0)
                                                break;
                                            else
                            ...
                            {
                                textoutput = textoutput + field.Name + delimeter + fieldValue + delimeter;
                            }
                        }
        ...
        public async Task setWPPartialUpdateAsync(byte sysid, byte compid, ushort startwp, ushort endwp,
            MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
            var req = new mavlink_mission_write_partial_list_t
            {
                target_system = sysid,
                target_component = compid,
        ...
        [Obsolete]
        public void setWPTotal(ushort wp_total, MAV_MISSION_TYPE type = MAV_MISSION_TYPE.MISSION)
        {
        ...
        public void InjectGpsData(byte sysid, byte compid, byte[] data, ushort length, bool rtcm_message = true)
        {
            ...
            if (rtcm_message)
            {
                var gps = new mavlink_gps_rtcm_data_t();
                var msglen = 180;
                ...
                for (var a = 0; a < nopackets; a++)
                {
                    ...
                    var copy = Math.Min(length - a * msglen, msglen);
            ...
            {
                var gps = new mavlink_gps_inject_data_t();
                var msglen = 110;
            ...
            byte autocontinue = 1, bool use_int = false, MAV_MISSION_TYPE mission_type = MAV_MISSION_TYPE.MISSION)
        {
            if (use_int)
            {
                var req = new mavlink_mission_item_int_t
                {
                    target_system = sysid,
                    target_component = compid,
                    command = loc.id,
        ...
        public void setGuidedModeWP(byte sysid, byte compid, Locationwp gotohere, bool setguidedmode = true)
        {
            ...
            try
            {
                ...
                {
                    var ans = setWP(sysid, compid, gotohere, 0, MAV_FRAME.GLOBAL_RELATIVE_ALT, 2);
                ...
                {
                    setPositionTargetGlobalInt(sysid, compid,
        ...
        public void setNewWPAlt(byte sysid, byte compid, Locationwp gotohere)
        {
            try
            {
                ...
                var ans = setWP(sysid, compid, gotohere, 0, MAV_FRAME.GLOBAL_RELATIVE_ALT, 3);
            ...
            double yawrate)
        {
            ...
            var pos_ignore = (target.type_mask & MAVLINK_SET_POS_TYPE_MASK_POS_IGNORE) > 0;
            var vel_ignore = (target.type_mask & MAVLINK_SET_POS_TYPE_MASK_VEL_IGNORE) > 0;
            var acc_ignore = (target.type_mask & MAVLINK_SET_POS_TYPE_MASK_ACC_IGNORE) > 0;
        ...
        {
            var req = new mavlink_digicam_control_t();

            req.target_system = MAV.sysid;
            req.target_component = MAV.compid;
            req.shot = shot ? (byte)1 : (byte)0;
        ...
        {
            var req = new mavlink_mount_configure_t();

            req.target_system = MAV.sysid;
            req.target_component = MAV.compid;
            req.mount_mode = (byte)mountmode;
            req.stab_pitch = stabpitch ? (byte)1 : (byte)0;
            req.stab_roll = stabroll ? (byte)1 : (byte)0;
            req.stab_yaw = stabyaw ? (byte)1 : (byte)0;
        ...
        {
            var req = new mavlink_mount_control_t
            {
                target_system = sysid,
                target_component = compid
            };
        ...
        public void setMode(byte sysid, byte compid, mavlink_set_mode_t mode, MAV_MODE_FLAG base_mode = 0)
        {
            ...
            // old
            generatePacket((byte)MAVLINK_MSG_ID.SET_MODE, mode, sysid, compid);
            Thread.Sleep(10);
            generatePacket((byte)MAVLINK_MSG_ID.SET_MODE, mode, sysid, compid);
        }
        ...
        {
            var buffer = new byte[MAVLINK_MAX_PACKET_LEN + 25];
            var count = 0;
            var length = 0;
            var readcount = 0;
            ...
            try
            {
                ...
                while ((BaseStream != null && BaseStream.IsOpen) || logreadmode)
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            var to = DateTime.Now.AddMilliseconds(BaseStream.ReadTimeout);
                    ...
                    {
                        log.Info("MAVLink readpacket read error: " + e);
                        break;
                    }
                    ...
                    if (buffer[0] == MAVLINK_STX_MAVLINK1 || buffer[0] == MAVLINK_STX || buffer[0] == 'U')
                    {
                        ...
                        if (count == 0 && !logreadmode)
                        {
                            ...
                            var read = BaseStream.Read(buffer, 1, headerlength);
                        ...
                        if (buffer[0] == MAVLINK_STX)
                        {
                            ...
                            if ((buffer[2] & MAVLINK_IFLAG_SIGNED) > 0) length += MAVLINK_SIGNATURE_BLOCK_LEN;
                        }
                        else
                        {
                            length = buffer[1] + headerlengthstx +
                        ...
                        if (count >= headerlength || logreadmode)
                        {
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        var start1 = DateTime.Now;
                                        var read = BaseStream.Read(buffer, headerlengthstx, length - headerlengthstx);
                                        var end = DateTime.Now - start1;
                                        var lapse = end.TotalMilliseconds;
                                        //Console.WriteLine("read: " + lapse);
                ...
                if (_bpstime.Second != DateTime.Now.Second)
                {
                    ...
                    else if (logreadmode) btr = logplaybackfile.BaseStream.Length - logplaybackfile.BaseStream.Position;
                ...
                var msginfo = MAVLINK_MESSAGE_INFOS.GetMessageInfo(msgid);

                // calc crc
                var sigsize = message.sig != null ? MAVLINK_SIGNATURE_BLOCK_LEN : 0;
                var crc = MavlinkCRC.crc_calculate(buffer, message.Length - sigsize - MAVLINK_NUM_CHECKSUM_BYTES);
                    ...
                    (message.crc16 & 0xff) != (crc & 0xff))
                {
                    if (buffer.Length > 5 && msginfo.name != null)
                        log.InfoFormat("Mavlink Bad Packet (crc fail) len {0} crc {1} vs {4} pkno {2} {3}",
                            buffer.Length,
                            crc, message.msgid, msginfo.name,
                            message.crc16);
                    if (logreadmode)
                ...
                if (!MAVlist.Contains(sysid, compid))
                {
                    ...
                    MAVlist[sysid, compid].recvpacketcount = unchecked(packetSeqNo - 1);
                }
                ...
                if (buffer.Length >= 5) getInfoFromStream(ref message, sysid, compid);
                    ...
                    buffer.Length >= message.payloadlength)
                {
                    ...
                    var expectedPacketSeqNo = (MAVlist[sysid, compid].recvpacketcount + 1) % 0x100;
            ...
            try
            {
                ...
                if (packetSeemValid)
                {
                    ...
                    {
                        var hb = message.ToStructure<mavlink_heartbeat_t>();
                    ...
                    {
                        var hl = message.ToStructure<mavlink_high_latency2_t>();
                    ...
                    if (msgid == (byte)MAVLINK_MSG_ID.STATUSTEXT) // status text
                    {
                        ...
                        // copter 3.4+
                        // plane 3.4+
                        var mavsev = MAV_SEVERITY.EMERGENCY;
                        ...
                        if (printit)
                        {
                            ...
                            if (Speech != null &&
                                ...
                                Settings.Instance["speechenable"] == "True")
                                if (speechenabled)
                    ...
                    if (Settings.Instance["autoParamCommit"] == null ||
                        Settings.Instance.GetBoolean("autoParamCommit"))
        ...
        {
            var type = (MAVLINK_MSG_ID)buffer.msgid;
            ...
            foreach (var item in list)
                ...
                if ((item.msgId == type && buffer.sysid == item.sysid && buffer.compid == item.compid) ||
                    (item.msgId == type && item.sysid == 0 && item.compid == 0 && buffer.sysid == sysidcurrent &&
                     buffer.compid == compidcurrent))
                    try
                    {
        ...
        private void getInfoFromStream(ref MAVLinkMessage buffer, byte sysid, byte compid)
        {
            ...
            {
                // clear old
                var wp = buffer.ToStructure<mavlink_mission_count_t>();
            ...
            {
                var rallypt = buffer.ToStructure<mavlink_rally_point_t>();
            ...
            {
                var camerapt = buffer.ToStructure<mavlink_camera_feedback_t>();
            ...
            {
                var fencept = buffer.ToStructure<mavlink_fence_point_t>();
            ...
            {
                var value = buffer.ToStructure<mavlink_param_value_t>();

                var st = Encoding.UTF8.GetString(value.param_id);
            ...
            {
                var now_ns =
                    (long)((DateTime.Now - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalMilliseconds *
                           1000000);

                var tsync = buffer.ToStructure<mavlink_timesync_t>();
                // tc1 - client
                // ts1 - server
                ...
                {
                    var offset_ns = (tsync.ts1 + now_ns - tsync.tc1 * 2) / 2;
                    var dt = MAVlist[buffer.sysid, buffer.compid].time_offset_ns - offset_ns;
            ...
            while (true)
            {
                ...
                if (buffer.Length > 5)
                    if (buffer.msgid == (byte)MAVLINK_MSG_ID.FENCE_POINT && buffer.sysid == req.target_system &&
                        buffer.compid == req.target_component)
                    {
                        var fp = buffer.ToStructure<mavlink_fence_point_t>();
        ...
        public async Task<string> GetLog(byte sysid, byte compid, ushort no)
        {
            ...
            using (var ms = new FileStream(filename, FileMode.Create, FileAccess.ReadWrite))
            {
                ...
                uint ofs = 0;
                uint bps = 0;
                var bpstimer = DateTime.Now;

                var req = new mavlink_log_request_data_t();
                ...
                while (true)
                {
                    ...
                    if (buffer.Length > 5)
                            ...
                            buffer.compid == req.target_component)
                        {
                            ...
                            {
                                if (Progress != null) Progress((int)req.ofs, "");
                ...
                while (true && ((BaseStream != null && BaseStream.IsOpen) || logreadmode))
                {
                    ...
                    if (!(start.AddMilliseconds(500) > DateTime.Now))
                        for (var a = 0; a < totallength / 90 + 1; a++)
                    ...
                    if (buffer.Length > 5)
                            ...
                            buffer.compid == req.target_component)
                        {
                            ...
                            if (set.ContainsKey((data.ofs / 90 + 1).ToString())) start = DateTime.MinValue;

                            // if data is less than max packet size or 0 > exit
                            if (data.count < 90 || data.count == 0) continue;
                        }
                }
        ...
        {
            var ans = new List<mavlink_log_entry_t>();

            return GetLogEntry().AwaitSync().Values
        ...
        {
            var req = new mavlink_mount_status_t();
            req.target_component = MAV.compid;
            req.target_system = MAV.sysid;
            ...
            ushort endno = ushort.MaxValue)
        {
            ...
            var ans = new Dictionary<ushort, mavlink_log_entry_t>();

            var req = new mavlink_log_request_list_t();
        ...
        {
            var req = new mavlink_log_erase_t();
        ...
        public async Task<List<PointLatLngAlt>> getRallyPoints()
        {
            ...
            var count = int.Parse(MAV.param["RALLY_TOTAL"].ToString());
        ...
        [Obsolete]
        public bool setRallyPoint(byte index, PointLatLngAlt plla, short break_alt, ushort land_dir_cd, byte flags,
            byte rallypointcount)
        {
            var rp = new mavlink_rally_point_t();
        ...
        public void send_text(byte severity, string txt)
        {
            ...
            generatePacket((byte)MAVLINK_MSG_ID.STATUSTEXT,
                new mavlink_statustext_t { severity = severity, text = temp });
        }
        ...
        private MAVLinkMessage readlogPacketMavlink()
        {
            ...
            if (!missingtimestamp)
            {
                var tem = logplaybackfile.BaseStream.Read(datearray, 0, datearray.Length);
            ...
            while (a < length)
            {
                ...
                var tempb = logplaybackfile.ReadByte();
                filepos++;
                ...
                switch (a)
                {
                    ...
                    case 1:
                        ...
                        // handle length
                    {
                        var headerlength = byte0 == MAVLINK_STX ? 9 : 5;
                        var headerlengthstx = headerlength + 1;
            ...
            var tmp = new MAVLinkMessage(temp, lastlogread);
        ...
        public bool translateMode(byte sysid, byte compid, string modein, ref mavlink_set_mode_t mode)
        {
            ...
            try
            {
                var modelist = Common.getModesList(MAVlist[sysid, compid].cs.firmware);
        ...
        public void uAvionixADSBControl(int baroAltMSL, ushort squawk, /*UAVIONIX_ADSB_OUT_CONTROL_STATE*/
            byte state, /*UAVIONIX_ADSB_EMERGENCY_STATUS*/byte emergencyStatus, byte[] flight_id, byte x_bit)
        {
            var ctrl = new mavlink_uavionix_adsb_out_control_t();
            ctrl.state = state;
