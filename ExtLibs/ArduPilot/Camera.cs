...
namespace MissionPlanner.Utilities
{
    public class Camera
    {
        public void test(MAVLinkInterface mavint)
        {
            try
            {
                var mav = mavint.MAVlist.FirstOrDefault(a =>
                    a.compid == (byte)MAVLink.MAV_COMPONENT.MAV_COMP_ID_CAMERA);

                if (mav == null)
                    return;

                mav.parent.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.REQUEST_CAMERA_INFORMATION, 0, 0, 0, 0, 0,
                    0, 0);
                mav.parent.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.REQUEST_VIDEO_STREAM_INFORMATION, 0, 0, 0,
                    0, 0, 0, 0);
                mav.parent.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.REQUEST_CAMERA_SETTINGS, 0, 0, 0, 0, 0, 0,
                    0);
                mav.parent.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.SET_CAMERA_MODE, 0, 0, 0, 0, 0, 0,
                    0); // p2 = 1 for recodring hint
                mav.parent.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.REQUEST_STORAGE_INFORMATION, 0, 0, 0, 0, 0,
                    0, 0);
        ...
        public void CameraInfo(MAVState mav, Action<MAVLink.mavlink_camera_information_t> act)
        {
            ...
            mav.parent.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.REQUEST_CAMERA_INFORMATION, 0, 0, 0, 0, 0, 0,
                0);
