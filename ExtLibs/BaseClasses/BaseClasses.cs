...
namespace DirectShow.BaseClasses
{
    ...
    public class RegisterFileExtension : Attribute
    {
        ...
        public RegisterFileExtension(string _extension, string _MediaType, string _SubType)
            : this(_extension)
        {
            if (!string.IsNullOrEmpty(_MediaType)) MediaType = new Guid(_MediaType);
            if (!string.IsNullOrEmpty(_SubType)) SubType = new Guid(_SubType);
        }
        ...
        public Guid MediaType { get; } = Guid.Empty;

        public Guid SubType { get; } = Guid.Empty;
    ...
    public class RegisterProtocolExtension : Attribute
    {
        ...
        public RegisterProtocolExtension(string _protocol, string _extension)
            : this(_protocol)
        {
            if (!string.IsNullOrEmpty(_extension)) Extensions.Add(_extension);
        }
    ...
    public class RegisterMediaType : Attribute
    {
        ...
        {
            if (!string.IsNullOrEmpty(_filter)) FilterGuid = new Guid(_filter);
            if (!string.IsNullOrEmpty(_majortype)) MajorType = new Guid(_majortype);
            if (!string.IsNullOrEmpty(_subtype)) SubType = new Guid(_subtype);
            if (!string.IsNullOrEmpty(_sequence)) Sequence = _sequence;
        }
        ...
        public Guid FilterGuid { get; } = typeof(DSFileSourceAsync).GUID;

        public Guid MajorType { get; } = MediaType.Stream;
    ...
    public class BaseEnum : COMHelper
    {
        ...
        public virtual int Skip(int cSkip)
        {
            ...
            if (m_iIndex + cSkip > m_iCount) return S_FALSE;
            m_iIndex += cSkip;
            return NOERROR;
        }
    ...
    public class EnumPins : BaseEnum, IEnumPins
    {
        ...
        public virtual int Next(int cPins, IPin[] ppPins, IntPtr pcFetched)
        {
            ...
            while (m_iIndex < m_iCount && _count < cPins)
            {
                ...
                lock ((m_Owner as BaseFilter).FilterLock)
                {
                    ppPins[_count++] = (m_Owner as BaseFilter).Pins[m_iIndex++];
                }
            }

            if (pcFetched != IntPtr.Zero) Marshal.WriteInt32(pcFetched, _count);
    ...
    public class EnumMediaTypes : BaseEnum, IEnumMediaTypes
    {
        ...
        {
            var _enum = new EnumMediaTypes((BasePin)m_Owner);
        ...
        public virtual int Next(int cMediaTypes, IntPtr ppMediaTypes, IntPtr pcFetched)
        {
            ...
            if (pcFetched != IntPtr.Zero)
                Marshal.WriteInt32(pcFetched, 0);
            else if (cMediaTypes > 1) return E_INVALIDARG;
            if (IsOutOfSync()) OnReset();
            var _count = 0;
            while (cMediaTypes > 0)
            {
                ...
                var hr = (m_Owner as BasePin).GetMediaType(m_iIndex++, ref mt);
                if (S_OK != hr) break;
                var _pmt = Marshal.AllocCoTaskMem(Marshal.SizeOf(typeof(AMMediaType)));
                if (_pmt == IntPtr.Zero)
                {
    ...
    public class PropPageSupport : COMHelper, ISpecifyPropertyPages
    {
        ...
        {
            var _attributes = Attribute.GetCustomAttributes(GetType(), typeof(PropPageSetup));
            if (_attributes != null)
                foreach (PropPageSetup _setup in _attributes)
                    if (_setup != null && _setup.Guids.Count > 0)
                        for (var i = 0; i < _setup.Guids.Count; i++)
        ...
        public List<Guid> Pages => m_Pages;
        ...
        public virtual int GetPages(out DsCAUUID pPages)
        {
            ...
            try
            {
                if (m_Pages.Count > 0)
                {
                    ...
                    var cb = Marshal.SizeOf(typeof(Guid));
                    pPages.pElems = Marshal.AllocCoTaskMem(cb * m_Pages.Count);
                    var _ptr = pPages.pElems;
    ...
    public abstract class BasePin : PropPageSupport, IPin, IQualityControl
    {
        ...
        protected BaseFilter m_Filter;
        protected string m_sName = "";
        protected object m_Lock;
        protected PinDirection m_Direction = PinDirection.Input;
        protected AMMediaType m_mt;
        protected bool m_bCanReconnectWhenActive;
        protected bool m_bTryMyTypesFirst;
        protected bool m_bRunTimeError;
        ...
        public IQualityControl QualitySync
        {
            get
            {
                if (m_QualitySink != IntPtr.Zero) return (IQualityControl)Marshal.GetObjectForIUnknown(m_QualitySink);
                return null;
            }
        }
        ...
        ~BasePin()
        {
            ...
            m_Filter = null;
            if (m_ConnectedPin != IntPtr.Zero) Marshal.Release(m_ConnectedPin);
        ...
        public virtual int CheckConnect(ref IPinImpl _pin)
        {
            ...
            var hr = (HRESULT)_pin.QueryDirection(out _direction);
        ...
        protected virtual int AgreeMediaType(ref IPinImpl pReceivePin, AMMediaType pmt)
        {
            ...
            if (pmt != null && !AMMediaType.IsPartiallySpecified(pmt)) return AttemptConnection(ref pReceivePin, pmt);
            int hrFailure = VFW_E_NO_ACCEPTABLE_TYPES;

            for (var i = 0; i < 2; i++)
            {
                int hr;
                var _ptr = IntPtr.Zero;
                if (i == (BOOL)m_bTryMyTypesFirst)
                {
                    hr = pReceivePin.EnumMediaTypes(out _ptr);
                    if (_ptr != IntPtr.Zero) pEnumMediaTypes = new IEnumMediaTypesImpl(_ptr);
                }
                else
                {
                    hr = EnumMediaTypes(out _ptr);
                    if (_ptr != IntPtr.Zero) pEnumMediaTypes = (IEnumMediaTypes)Marshal.GetObjectForIUnknown(_ptr);
                }
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (Marshal.IsComObject(pEnumMediaTypes)) Marshal.ReleaseComObject(pEnumMediaTypes);
                    if (SUCCEEDED(hr)) return NOERROR;

                    if (hr != E_FAIL && hr != E_INVALIDARG && hr != VFW_E_TYPE_NOT_ACCEPTED) hrFailure = hr;
                }
            }
        ...
        {
            var hr = CheckConnect(ref pReceivePin);
            if (FAILED(hr))
            {
            ...
            if (hr == NOERROR)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    var _ptr = Marshal.GetIUnknownForObject(this);
                    var _guid = typeof(IPin).GUID;
                    IntPtr _this;
            ...
            {
                if (SUCCEEDED(hr) || hr == E_FAIL || hr == E_INVALIDARG) hr = VFW_E_TYPE_NOT_ACCEPTED;
            }
        ...
        protected virtual int TryMediaTypes(ref IPinImpl pReceivePin, AMMediaType pmt, IEnumMediaTypes pEnum)
        {
            ...
            try
            {
                for (;;)
                {
                    ...
                    if (hr != S_OK)
                    {
                        if (S_OK == hrFailure) hrFailure = VFW_E_NO_ACCEPTABLE_TYPES;
                        return hrFailure;
                    }
        ...
        public virtual int ReceiveConnection(IntPtr pReceivePin, AMMediaType pmt)
        {
            ...
            lock (m_Lock)
            {
                var _pin = new IPinImpl(pReceivePin);
                if (m_ConnectedPin != IntPtr.Zero) return VFW_E_ALREADY_CONNECTED;
                if (!IsStopped && !m_bCanReconnectWhenActive) return VFW_E_NOT_STOPPED;
                var hr = CheckConnect(ref _pin);
                if (FAILED(hr))
                {
                    ASSERT(SUCCEEDED(BreakConnect()));
                    return hr;
        ...
        public virtual int Connect(IntPtr pReceivePin, AMMediaType pmt)
        {
            ...
            lock (m_Lock)
            {
                ...
                if (!IsStopped && !m_bCanReconnectWhenActive) return VFW_E_NOT_STOPPED;
                var _pin = new IPinImpl(pReceivePin);
                var hr = AgreeMediaType(ref _pin, pmt);
                if (FAILED(hr))
                {
                    ASSERT(SUCCEEDED(BreakConnect()));
                    return hr;
        ...
        public virtual int ConnectedTo(out IntPtr ppPin)
        {
            ...
            if (ppPin != IntPtr.Zero) Marshal.AddRef(ppPin);
        ...
        {
            int hr = S_OK;
            var _enum = new EnumMediaTypes(this);
            ppEnum = Marshal.GetComInterfaceForObject(_enum, typeof(IEnumMediaTypes));
            return hr;
        }
        ...
        public virtual int SetSink(IntPtr piqc)
        {
            lock (m_Lock)
            {
                if (m_QualitySink != IntPtr.Zero) Marshal.Release(m_QualitySink);
                m_QualitySink = piqc;
                if (m_QualitySink != IntPtr.Zero) Marshal.AddRef(m_QualitySink);
            }
        ...
        protected static HRESULT CreatePosPassThru(IntPtr _owner, IPin pPin, bool bRenderer, out IntPtr ppPassThru)
        {
            ...
            var _seek = new ISeekingPassThruImpl(ppPassThru);
    ...
    public abstract class BaseInputPin : BasePin, IMemInputPin
    {
        ...
        public override int Inactive()
        {
            ...
            if (m_pAllocator == IntPtr.Zero) return VFW_E_NO_ALLOCATOR;
        ...
        public virtual int CheckStreaming()
        {
            ...
            if (m_bRunTimeError) return VFW_E_RUNTIME_ERROR;
            return S_OK;
        }
        ...
        {
            if (m_QualitySink != IntPtr.Zero)
                return QualitySync.Notify(Marshal.GetIUnknownForObject(m_Filter), q);
            try
            {
                int hr = VFW_E_NOT_FOUND;
                if (m_ConnectedPin != IntPtr.Zero)
                {
                    var pIQC = (IQualityControl)Marshal.GetObjectForIUnknown(m_ConnectedPin);
                    if (pIQC != null)
                    {
                        hr = pIQC.Notify(Marshal.GetIUnknownForObject(m_Filter), q);
                        if (Marshal.IsComObject(pIQC)) Marshal.ReleaseComObject(pIQC);
                        pIQC = null;
                    }
                }
        ...
        {
            var hr = CheckStreaming();
            if (S_OK != hr) return hr;
            var _guid = typeof(IMediaSample2).GUID;
            IntPtr pSample2;
            if (S_OK == _sample._QueryInterface(ref _guid, out pSample2))
            {
                var _sample2 = new IMediaSample2Impl(pSample2);
                var pStructure = Marshal.AllocCoTaskMem(Marshal.SizeOf(typeof(AMSample2Properties)));
            ...
            else
            {
                ...
                if (S_OK == _sample.IsPreroll()) m_SampleProps.dwSampleFlags |= AMSamplePropertyFlags.PreRoll;
                if (S_OK == _sample.IsSyncPoint()) m_SampleProps.dwSampleFlags |= AMSamplePropertyFlags.SplicePoint;
            ...
            if ((m_SampleProps.dwSampleFlags & AMSamplePropertyFlags.TypeChanged) == 0) return NOERROR;
        ...
        {
            var _sample = new IMediaSampleImpl(pSample);
        ...
        public virtual int ReceiveMultiple(IntPtr pSamples, int nSamples, out int nSamplesProcessed)
        {
            ...
            while (nSamples-- > 0)
            {
                var _sample = Marshal.ReadIntPtr(pSamples, nSamples * IntPtr.Size);
                hr = Receive(_sample);
                if (hr != S_OK) break;
                nSamplesProcessed++;
            }
        ...
        {
            var cOutputPins = 0;
            for (var i = 0; i < m_Filter.Pins.Count; i++)
            {
                var _pin = m_Filter.Pins[i];
                if (_pin.Direction == PinDirection.Output)
                {
                    IntPtr pPtr;
                    var hr = _pin.ConnectedTo(out pPtr);
                    if (SUCCEEDED(hr))
                    {
                        cOutputPins++;
                        var _guid = typeof(IMemInputPin).GUID;
                        IntPtr pInputPin;
                        hr = Marshal.QueryInterface(pPtr, ref _guid, out pInputPin);
                        if (SUCCEEDED(hr))
                        {
                            var _input = new IMemInputPinImpl(pInputPin);
    ...
    public abstract class BaseOutputPin : BasePin
    {
        ...
        public virtual int ReconnectPin()
        {
            ...
            {
                var hr = E_FAIL;
                var pGraph2 = (IFilterGraph2)m_Filter.FilterGraph;
                if (pGraph2 != null)
                {
                    var nIndex = 0;
                    AMMediaType pmt;
                ...
                if (!hr.Succeeded) hr = (HRESULT)m_Filter.FilterGraph.Reconnect(this);
                return hr;
            }
        ...
        {
            var hr = base.CheckConnect(ref _pin);
            if (FAILED(hr)) return hr;
            var _guid = typeof(IMemInputPin).GUID;
        ...
        public virtual int DecideAllocator(IMemInputPinImpl pPin, out IntPtr ppAlloc)
        {
            int hr = NOERROR;
            ppAlloc = IntPtr.Zero;
            var prop = new AllocatorProperties();
            prop.cbAlign = 0;
            prop.cbBuffer = 0;
            prop.cbPrefix = 0;
            prop.cBuffers = 0;
            ...
            if (SUCCEEDED(hr))
            {
                var _allocator = new IMemAllocatorImpl(ppAlloc);
            ...
            AMGBF dwFlags)
        {
            if (m_pAllocator != IntPtr.Zero)
            {
                var _start = IntPtr.Zero;
                var _stop = IntPtr.Zero;
        ...
        {
            if (m_pInputPin == IntPtr.Zero) return VFW_E_NOT_CONNECTED;
            return InputPin.Receive(pSample);
        }

        public virtual int DeliverEndOfStream()
        {
            if (m_ConnectedPin == IntPtr.Zero) return VFW_E_NOT_CONNECTED;
    ...
    public abstract class BaseFilter : PropPageSupport, IPersistStream, IBaseFilter
    {
        ...
        protected long m_tStart;
        protected bool m_bPersistDirty;
        ...
        public virtual int NotifyEvent(EventCode _code, IntPtr _param1, IntPtr _param2)
        {
            ...
            if (pSink != null)
            {
                var _pUnknown = Marshal.GetIUnknownForObject(this);
                try
                {
                    if (_code == EventCode.Complete) _param2 = _pUnknown;
        ...
        public virtual int ReconnectPin(IPin pPin, AMMediaType pmt)
        {
            ...
            {
                int hr;
                var pGraph2 = (IFilterGraph2)FilterGraph;
                if (pGraph2 != null)
                {
        ...
        public virtual int StreamTime(out long rtStream)
        {
            ...
            var hr = Clock.GetTime(out rtStream);
            if (FAILED(hr)) return hr;
        ...
        public virtual int GetSyncSource(out IntPtr pClock)
        {
            ...
            {
                pClock = m_pClock;
                if (m_pClock != IntPtr.Zero) Marshal.AddRef(m_pClock);
            }
        ...
        public virtual int SetSyncSource(IntPtr pClock)
        {
            lock (m_Lock)
            {
                if (m_pClock != IntPtr.Zero) Marshal.Release(m_pClock);
                m_pClock = pClock;
                if (m_pClock != IntPtr.Zero) Marshal.AddRef(m_pClock);
            }
        ...
        public virtual int JoinFilterGraph(IntPtr pGraph, string pName)
        {
            lock (m_Lock)
            {
                ...
                if (m_pGraph != IntPtr.Zero) Marshal.Release(m_pGraph);
                m_pGraph = pGraph;
                if (m_pGraph != IntPtr.Zero)
                {
                    Marshal.AddRef(m_pGraph);
                    var _guid = typeof(IMediaEventSink).GUID;
                    Marshal.QueryInterface(m_pGraph, ref _guid, out m_pSink);
                }

                if (pName != null) m_sName = pName;
            }
            ...
            lock (m_Lock)
            {
                ...
                if (m_State != FilterState.Stopped)
                {
                    ...
                    for (var i = 0; i < m_Pins.Count; i++)
                        if (m_Pins[i].IsConnected)
                        {
                            var hrTmp = m_Pins[i].Inactive();
                            if (FAILED(hrTmp) && SUCCEEDED(hr)) hr = hrTmp;
                        }
                }
        ...
        public virtual int Load(IntPtr pStm)
        {
            ...
            try
            {
                ...
                if (hr.Succeeded) SetDirty(false);
            }
        ...
        public virtual int Save(IntPtr pStm, bool fClearDirty)
        {
            ...
            try
            {
                ...
                if (hr.Succeeded && fClearDirty) SetDirty(false);
            }
            catch (Exception _exception)
            {
                hr = (HRESULT)Marshal.GetHRForException(_exception);
        ...
        protected virtual long SizeMax()
        {
            ...
            try
            {
                ...
                var hr = WriteToStream(_stream);
                if (hr == S_OK) _size = _stream.Length;
            }
            finally
            {
                _stream.Dispose();
        ...
        protected object GetFilterRegistryValue(string _name, object _default)
        {
            if (!string.IsNullOrEmpty(_name))
            {
                ...
                try
                {
                    var _path = "CLSID\\" + GetType().GUID.ToString("B") + "\\" + m_csRegistryPath;
                    _key = Registry.ClassesRoot.CreateSubKey(_path);
                    return _key.GetValue(_name, _default);
                }
                catch (Exception _exception)
                {
                    var hr = (HRESULT)Marshal.GetHRForException(_exception);
                    hr.TraceWrite();
                }
                finally
                {
                    if (_key != null) _key.Close();
                }
            }
        ...
        public static void RegisterFunction(Type _type)
        {
            if (_type.IsClass)
            {
                var _setup = (AMovieSetup)Attribute.GetCustomAttribute(_type, typeof(AMovieSetup));
                if (_setup != null && _setup.ShouldRegister)
                    if (_type.IsSubclassOf(typeof(BaseFilter)) || (_setup.Name != null && _setup.Name != ""))
                    {
                        var _filter = (BaseFilter)Activator.CreateInstance(_type);
                        ...
                        if (_setup.Category != null && _setup.Category != Guid.Empty &&
                            _setup.Category != FilterCategory.LegacyAmFilterCategory)
                            _category = new DsGuid(_setup.Category);
                        var _mapper2 = (IFilterMapper2)new FilterMapper2();
                        ASSERT(_mapper2);
                        if (_mapper2 != null)
                        {
                            var hr = _mapper2.UnregisterFilter(_category, _instance, _type.GUID);

                            var _reg2 = new RegFilter2();
                            _reg2.dwVersion = _setup.Version;
                            ...
                            if (SUCCEEDED(hr))
                            {
                                var _register = Marshal.AllocCoTaskMem(Marshal.SizeOf(_reg2));
                                Marshal.StructureToPtr(_reg2, _register, true);

                                hr = _mapper2.RegisterFilter(_type.GUID, _name, IntPtr.Zero, _category, _instance,
                                    _register);
                                if (E_FILE_NOT_FOUND == hr) hr = NOERROR;
            ...
            if (_type.IsClass)
            {
                ...
                if (_setup != null && _setup.ShouldRegister)
                    if (_type.IsSubclassOf(typeof(BaseFilter)) || (_setup.Name != null && _setup.Name != ""))
                    {
                        ...
                        if (_mapper2 != null)
                        {
                            var hr = _filter.BeforeUninstall(ref _mapper2);
                        ...
                        try
                        {
                            var _path = "CLSID\\" + _type.GUID.ToString("B");
                            _key = Registry.ClassesRoot.CreateSubKey(_path);
                            if (_key != null) _key.DeleteSubKeyTree(m_csRegistryPath);
                        }
                        catch
                        {
                        }
    ...
    public class TransformInputPin : BaseInputPin
    {
        ...
        public override int CheckConnect(ref IPinImpl _pin)
        {
            var hr = ((TransformFilter)m_Filter).CheckConnect(PinDirection.Input, ref _pin);
            if (FAILED(hr)) return hr;
        ...
        public override int CompleteConnect(ref IPinImpl pReceivePin)
        {
            var hr = ((TransformFilter)m_Filter).CompleteConnect(PinDirection.Input, ref pReceivePin);
            if (FAILED(hr)) return hr;
        ...
        {
            var hr = ((TransformFilter)m_Filter).CheckInputType(mt);
            if (S_OK != hr) return hr;
        ...
        public override int CheckStreaming()
        {
            ...
            if (!((TransformFilter)m_Filter).Output.IsConnected) return VFW_E_NOT_CONNECTED;
        ...
        public override int OnReceive(ref IMediaSampleImpl _sample)
        {
            ...
            lock (((TransformFilter)m_Filter).ReceiveLock)
            {
                ...
                if (S_OK == hr) hr = ((TransformFilter)m_Filter).OnReceive(ref _sample);
        ...
        public override int EndOfStream()
        {
            ...
            {
                var hr = CheckStreaming();
                if (S_OK == hr) hr = ((TransformFilter)m_Filter).EndOfStream();
                return hr;
            }
        }
        ...
        public override int EndFlush()
        {
            lock (m_Filter.FilterLock)
            {
                ...
                var hr = ((TransformFilter)m_Filter).EndFlush();
                if (FAILED(hr)) return hr;
    ...
    public class TransformOutputPin : BaseOutputPin, IMediaSeeking
    {
        ...
        public override int CheckConnect(ref IPinImpl _pin)
        {
            ...
            if (((TransformFilter)m_Filter).Input.IsConnected == false) return E_UNEXPECTED;

            var hr = ((TransformFilter)m_Filter).CheckConnect(PinDirection.Output, ref _pin);
            if (FAILED(hr)) return hr;
            return base.CheckConnect(ref _pin);
        }
        ...
        public override int CompleteConnect(ref IPinImpl pReceivePin)
        {
            var hr = ((TransformFilter)m_Filter).CompleteConnect(PinDirection.Output, ref pReceivePin);
            if (FAILED(hr)) return hr;
            return base.CompleteConnect(ref pReceivePin);
        }
        ...
        public override int CheckMediaType(AMMediaType mt)
        {
            ...
            ASSERT(((TransformFilter)m_Filter).Input != null);
            if (!((TransformFilter)m_Filter).Input.IsConnected) return E_INVALIDARG;
        ...
        public override int Notify(IntPtr pSelf, Quality q)
        {
            var hr = ((TransformFilter)m_Filter).AlterQuality(q);
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.CheckCapabilities(ref pCapabilities);
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.ConvertTimeFormat(out pTarget, pTargetFormat, Source, pSourceFormat);
            pTarget = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetAvailable(out pEarliest, out pLatest);
            pLatest = 0;
            pEarliest = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetCapabilities(out pCapabilities);
            pCapabilities = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            IMediaSeeking _seeking = Seeking;
            if (_seeking != null) return _seeking.GetCurrentPosition(out pCurrent);
            pCurrent = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetDuration(out pDuration);
            pDuration = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetPositions(out pCurrent, out pStop);
            pCurrent = 0;
            pStop = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetPreroll(out pllPreroll);
            pllPreroll = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetRate(out pdRate);
            pdRate = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetStopPosition(out pStop);
            pStop = 0;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.GetTimeFormat(out pFormat);
            pFormat = Guid.Empty;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.IsFormatSupported(pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.IsUsingTimeFormat(pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.QueryPreferredFormat(out pFormat);
            pFormat = Guid.Empty;
            return E_NOINTERFACE;
        }

        public int SetPositions(DsLong pCurrent, AMSeekingSeekingFlags dwCurrentFlags, DsLong pStop,
            AMSeekingSeekingFlags dwStopFlags)
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.SetPositions(pCurrent, dwCurrentFlags, pStop, dwStopFlags);
            ;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.SetRate(dRate);
            ;
            return E_NOINTERFACE;
        }
        ...
        {
            var _seeking = Seeking;
            if (_seeking != null) return _seeking.SetTimeFormat(pFormat);
            ;
            return E_NOINTERFACE;
        }
    ...
    public abstract class TransformFilter : BaseFilter
    {
        ...
        protected bool m_bEOSDelivered;
        protected bool m_bQualityChanged;
        protected bool m_bSampleSkipped;
        ...
        public TransformInputPin Input
        {
            get
            {
                if (Pins.Count >= 1) return (TransformInputPin)Pins[0];
                return null;
            }
        }
        ...
        public TransformOutputPin Output
        {
            get
            {
                if (Pins.Count >= 2) return (TransformOutputPin)Pins[1];
                return null;
            }
        }
        ...
        public override int Pause()
        {
            lock (m_Lock)
            {
                ...
                else if (Input == null || !Input.IsConnected)
                {
        ...
        public override int Stop()
        {
            lock (m_Lock)
            {
                if (m_State == FilterState.Stopped) return NOERROR;
        ...
        {
            int hr = NOERROR;
            if (Output != null) hr = Output.DeliverBeginFlush();
            return hr;
        }
        ...
        {
            int hr = NOERROR;
            if (Output != null) hr = Output.DeliverEndOfStream();
            return hr;
        }
        ...
        {
            int hr = NOERROR;
            if (Output != null) hr = Output.DeliverNewSegment(tStart, tStop, dRate);
            return hr;
        }
        ...
        {
            var pProps = Input.SampleProps;
            var dwFlags = m_bSampleSkipped ? AMGBF.PrevFrameSkipped : 0;
            ppOutSample = null;
            if ((pProps.dwSampleFlags & AMSamplePropertyFlags.SplicePoint) == 0) dwFlags |= AMGBF.NotAsyncPoint;
            IntPtr pOutputSample;
            var hr = Output.GetDeliveryBuffer(
                out pOutputSample
            ...
            {
                var _sample2 = new IMediaSample2Impl(pSample2);
                var cb = Marshal.SizeOf(typeof(AMSample2Properties));
                var _properties = Marshal.AllocCoTaskMem(cb);
                if (SUCCEEDED(_sample2.GetProperties(cb, _properties)))
                {
                    var OutProps =
                        (AMSample2Properties)Marshal.PtrToStructure(_properties, typeof(AMSample2Properties));
                ...
                if ((pProps.dwSampleFlags & AMSamplePropertyFlags.DataDiscontinuity) > 0) m_bSampleSkipped = false;
                _sample2._Release();
            }
            else
            {
                if ((pProps.dwSampleFlags & AMSamplePropertyFlags.TimeValid) > 0)
                    ppOutSample.SetTime(pProps.tStart, pProps.tStop);
                if ((pProps.dwSampleFlags & AMSamplePropertyFlags.SplicePoint) > 0) ppOutSample.SetSyncPoint(true);
                ...
                if (_sample.GetMediaTime(out MediaStart, out MediaEnd) == NOERROR)
                    ppOutSample.SetMediaTime(MediaStart, MediaEnd);
            }
        ...
        public virtual int OnReceive(ref IMediaSampleImpl _sample)
        {
            ...
            try
            {
                if (pProps.dwStreamId != (int)AMSamplePropertyFlags.Media) return Output.Deliver(ref _sample);
                var hr = InitializeOutputSample(ref _sample, out pOutSample);
            ...
            {
                if (pOutSample != null) pOutSample._Release();
            }
        }
    ...
    public class TransInPlaceInputPin : TransformInputPin
    {
        ...
        public override int CheckMediaType(AMMediaType mt)
        {
            var hr = ((TransInPlaceFilter)m_Filter).CheckInputType(mt);
        ...
        {
            var hr = base.CompleteConnect(ref pReceivePin);
            if (FAILED(hr)) return hr;
        ...
        public override int GetAllocator(out IntPtr ppAllocator)
        {
            lock (m_Lock)
            {
                ...
                if (((TransInPlaceFilter)m_Filter).Output.IsConnected)
                {
                    ...
                    if (SUCCEEDED(hr)) ((TransInPlaceFilter)m_Filter).Output.AllocatorPtr = ppAllocator;
                }
                else
                {
        ...
        public override int NotifyAllocator(IntPtr pAllocator, bool bReadOnly)
        {
            lock (m_Lock)
            {
                ...
                {
                    var pOutputAllocator = ((TransInPlaceFilter)m_Filter).Output.AllocatorPtr;
                    if (pOutputAllocator == IntPtr.Zero)
                    {
                        hr = ((TransInPlaceFilter)m_Filter).Output.InputPin.GetAllocator(out pOutputAllocator);
                        if (FAILED(hr)) hr = CreateMemoryAllocator(out pOutputAllocator);
                        if (SUCCEEDED(hr)) ((TransInPlaceFilter)m_Filter).Output.AllocatorPtr = pOutputAllocator;
                    }

                    if (pAllocator == pOutputAllocator) return E_FAIL;
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            hr = ((IMemAllocator)Marshal.GetObjectForIUnknown(pOutputAllocator)).SetProperties(Props,
                                Actual);
                            GC.Collect();
                        }
                        ...
                        if (SUCCEEDED(hr))
                            hr = ((TransInPlaceFilter)m_Filter).Output.InputPin.NotifyAllocator(pOutputAllocator,
                                false);
                    }
                }
                else
                {
                    ...
                    if (SUCCEEDED(hr)) ((TransInPlaceFilter)m_Filter).Output.AllocatorPtr = pAllocator;
                }

                if (SUCCEEDED(hr)) AllocatorPtr = pAllocator;
                return hr;
            }
        }
    ...
    public abstract class TransInPlaceFilter : TransformFilter
    {
        ...
        protected int Copy(ref IMediaSampleImpl pSource, out IMediaSampleImpl pDest)
        {
            ...
            int hr;
            long tStart, tStop;
            var bTime = S_OK == pSource.GetTime(out tStart, out tStop);
            ...
            if (S_OK == pDest._QueryInterface(ref _guid, out pSample2))
            {
                ...
                var cb = Marshal.SizeOf(Input.SampleProps);
                var _properties = Marshal.AllocCoTaskMem(cb);
            ...
            {
                if (bTime) pDest.SetTime(tStart, tStop);

                if (S_OK == pSource.IsSyncPoint()) pDest.SetSyncPoint(true);
                if (S_OK == pSource.IsDiscontinuity() || m_bSampleSkipped) pDest.SetDiscontinuity(true);
                if (S_OK == pSource.IsPreroll()) pDest.SetPreroll(true);
            ...
            if (pSource.GetMediaTime(out TimeStart, out TimeEnd) == NOERROR) pDest.SetMediaTime(TimeStart, TimeEnd);

            {
                var lDataLength = pSource.GetActualDataLength();
                pDest.SetActualDataLength(lDataLength);
                // Copy the sample data
                {
                    IntPtr pSourceBuffer, pDestBuffer;
                    var lSourceSize = pSource.GetSize();
                    var lDestSize = pDest.GetSize();
        ...
        public bool ModifiesData => m_bModifiesData;

        public bool UsingDifferentAllocators => Input.Allocator != Output.Allocator;

        public bool TypesMatch => AMMediaType.AreEquals(Input.CurrentMediaType, Output.CurrentMediaType);
        ...
        public override int CompleteConnect(PinDirection _direction, ref IPinImpl pPin)
        {
            ...
            if (m_pGraph == null) return VFW_E_NOT_IN_GRAPH;

            if (_direction == PinDirection.Output)
            {
                if (Input.IsConnected) return ReconnectPin(Input, Output.CurrentMediaType);
                return NOERROR;
            }
        ...
        public override int OnReceive(ref IMediaSampleImpl _sample)
        {
            ...
            try
            {
                if (UsingDifferentAllocators)
                {
                    ...
                    if (hr != S_OK) return hr;
                    if (_output == null) return E_UNEXPECTED;
                }
                else
                {
                    _output = _sample;
            ...
            {
                if (_output != null && UsingDifferentAllocators) _output._Release();
            }

            return hr;
        }

        public sealed override int Transform(ref IMediaSampleImpl pIn, ref IMediaSampleImpl pOut)
        {
            return E_NOTIMPL;
        }
    ...
    public abstract class RenderedInputPin : BaseInputPin
    {
        ...
        protected bool m_bAtEndOfStream;
        protected bool m_bCompleteNotified;
        ...
        public override int EndOfStream()
        {
            ...
            if (S_OK == hr && !m_bAtEndOfStream)
            {
                ...
                if (fs == FilterState.Running) DoCompleteHandling();
            }
        ...
        {
            m_bCompleteNotified = false;
            if (m_bAtEndOfStream) DoCompleteHandling();
            return S_OK;
        }
    ...
    public abstract class AMThread : COMHelper, IDisposable
    {
        ...
        public int RequestParam { get; private set; }
        ...
        public string Name
        {
            ...
            set
            {
                if (value != null && value != m_sName && m_sName == "" && !IsExists()) m_sName = value;
            }
        }
        ...
        public int CallWorker(int dwParam)
        {
            lock (m_AccessLock)
            {
                if (!ThreadExists) return E_FAIL;
                RequestParam = dwParam;
        ...
        {
            if (!m_EventSend.WaitOne(0, false)) return false;
    ...
    public sealed class ManagedThread : AMThread
    {
        ...
        protected override bool OnCreate()
        {
            ...
            m_WorkerThread = new Thread(MainThreadProc);
            if (!string.IsNullOrEmpty(m_sName)) m_WorkerThread.Name = m_sName;
        ...
        protected override bool OnClose(int _timeout)
        {
            ...
            {
                if (m_WorkerThread.IsAlive) bResult = m_WorkerThread.Join(_timeout);
                if (bResult) m_WorkerThread = null;
            }
    ...
    public abstract class SourceStream : BaseOutputPin, IDisposable
    {
        ...
        public override int CheckMediaType(AMMediaType pmt)
        {
            lock (m_Filter.FilterLock)
            {
                ...
                {
                    GetMediaType(ref mt);
                    if (AMMediaType.AreEquals(mt, pmt)) return NOERROR;
                }
                finally
                {
                    AMMediaType.Free(ref mt);
        ...
        public int Init()
        {
            return m_Thread.CallWorker((int)Command.CMD_INIT);
        }

        public int Exit()
        {
            return m_Thread.CallWorker((int)Command.CMD_EXIT);
        }

        public int Run()
        {
            return m_Thread.CallWorker((int)Command.CMD_RUN);
        }

        public int Pause()
        {
            return m_Thread.CallWorker((int)Command.CMD_PAUSE);
        }

        public int Stop()
        {
            return m_Thread.CallWorker((int)Command.CMD_STOP);
        }

        protected Command GetRequest()
        {
            return (Command)m_Thread.GetRequest();
        }
        ...
        protected virtual int DoBufferProcessingLoop()
        {
            ...
            do
            {
                while (!CheckRequest(ref com))
                {
                    ...
                    var hr = GetDeliveryBuffer(out pSample, _start, _stop, AMGBF.NoWait);
        ...
        protected virtual int ThreadProc()
        {
            ...
            {
                com = GetRequest();
                if (com != Command.CMD_INIT) m_Thread.Reply(E_UNEXPECTED);
    ...
    public class OutputQueue : COMHelper, IDisposable
    {
        ...
        protected virtual void ThreadProc()
        {
            ...
            var _pin = new IMemInputPinImpl(pMemInputPin);
            Packet _packet;
            var hr = NOERROR;
            while (true)
            {
                var nWait = WaitHandle.WaitAny(new WaitHandle[] { m_evReady, m_evShutdown });
                if (nWait != 0)
                {
                    lock (m_csPacketsLock)
                    {
                        while (m_Packets.Count > 0)
                        {
                            ...
                            if (_packet.Type == PacketType.Sample) Marshal.Release(_packet.Data);
                        }
                    }
                ...
                bool bReset;
                var bFlushing = m_Flushing.WaitOne(0, false);
                lock (m_csPacketsLock)
                {
                    bReset = m_bReset;
                ...
                lock (m_csPopLock)
                {
                    if (m_hEventPop != null) m_hEventPop.Set();
                }
                ...
                if (_packet.Type == PacketType.EOS && !bReset)
                {
                    ...
                    if (hr.Failed) TRACE("EndOfStream() " + hr);
                }

                if (_packet.Type == PacketType.NewSegment && !bReset)
                {
                    var _segment = (NewSegmentPacket)Marshal.PtrToStructure(_packet.Data, typeof(NewSegmentPacket));
        ...
        protected class Packet
        {
            ...
            {
                var _data = new NewSegmentPacket();
                _data.tStart = _start;
                _data.tStop = _stop;
                _data.dRate = _rate;
        ...
        protected EventWaitHandle m_hEventPop;
        protected AMThread m_Thread;
        ...
        public OutputQueue(IntPtr pOutputPin)
        {
            ...
            m_Thread = new ManagedThread(ThreadProc);
            m_Thread.Create();
        }
        ...
        public IPinImpl OutputPin => new IPinImpl(m_pOutputPin);
    ...
    public class BasePropertyPage : Form, IPropertyPage
    {
        ...
        public static bool ShowPropertyPages(IBaseFilter _filter, IntPtr _hwnd)
        {
            try
            {
                var pProp = (ISpecifyPropertyPages)_filter;
                if (pProp != null)
                {
                    DsCAUUID pCAUUID;
                    if (COMHelper.SUCCEEDED(pProp.GetPages(out pCAUUID)))
                    {
                        object oDevice = _filter;
                        var hr = OleCreatePropertyFrame(_hwnd, 0, 0, "Filter ", 1, ref oDevice, pCAUUID.cElems,
                            pCAUUID.pElems, 0, 0, IntPtr.Zero);
        ...
        protected IPropertyPageSiteImpl m_pPageSite;
        protected bool m_bDirty;
        protected bool m_bObjectSet;
        ...
        public bool Dirty
        {
            ...
            set
            {
                ...
                {
                    m_bDirty = value;
                    if (m_pPageSite != null) m_pPageSite.OnStatusChange(m_bDirty ? PropStatus.Dirty : PropStatus.Clean);
                }
            }
        }
        ...
        public virtual int SetPageSite(IntPtr pPageSite)
        {
            ...
            {
                if (m_pPageSite != null) m_pPageSite._Release();
                m_pPageSite = null;
            }
            else
            {
            ...
            if (m_pPageSite != null && m_bDirty) m_pPageSite.OnStatusChange(PropStatus.Dirty);
            return COMHelper.NOERROR;
        }

        public virtual int Activate(IntPtr hWndParent, DsRect pRect, bool bModal)
        {
            if (pRect == null) return COMHelper.E_POINTER;
            if (m_bObjectSet == false) return COMHelper.E_UNEXPECTED;
            if (hWndParent == IntPtr.Zero) return COMHelper.E_INVALIDARG;
            CreateControl();
        ...
        public new virtual int Deactivate()
        {
            OnDeactivate();
            Hide();
        ...
        {
            if (pPageInfo == null) return COMHelper.E_POINTER;
            pPageInfo.cb = Marshal.SizeOf(typeof(PropPageInfo));
            pPageInfo.pszTitle = Marshal.StringToCoTaskMemAuto(Title);
            pPageInfo.pszDocString = IntPtr.Zero;
            pPageInfo.pszHelpFile = IntPtr.Zero;
            pPageInfo.dwHelpContext = 0;
            pPageInfo.size.cx = Width;
            pPageInfo.size.cy = Height;
        ...
        public virtual int SetObjects(uint cObjects, IntPtr ppUnk)
        {
            if (cObjects == 1)
            {
                if (m_bObjectSet) return COMHelper.E_UNEXPECTED;

                if (ppUnk == IntPtr.Zero) return COMHelper.E_POINTER;
                var pObject = Marshal.ReadIntPtr(ppUnk);
                if (pObject == IntPtr.Zero) return COMHelper.E_POINTER;
            ...
            if (cObjects == 0)
            {
                if (!m_bObjectSet) return COMHelper.NOERROR;
                // Set a flag to say that we have not set the Object for the page
                m_bObjectSet = false;
                return OnDisconnect();
            }

            return COMHelper.E_UNEXPECTED;
        }
        ...
        public new virtual int Move(DsRect pRect)
        {
        ...
        public virtual int Apply()
        {
            ...
            if (m_pPageSite == null) return COMHelper.E_UNEXPECTED;

            // Has anything changed

            if (m_bDirty == false) return COMHelper.NOERROR;

            // Commit derived class changes
            var hr = OnApplyChanges();
    ...
    public class PosPassThru : COMHelper, IMediaSeeking, IMediaPosition
    {
        ...
        public PosPassThru(ref HRESULT hr, IntPtr pPin)
        {
            hr = S_OK;
            m_pPin = pPin;
            if (m_pPin == IntPtr.Zero) hr = E_POINTER;
        }
        ...
        private IMediaSeekingImpl m_pSeeking;
        ...
        protected IMediaSeekingImpl Seeking
        {
            get
            {
                lock (m_csLock)
                {
                    if (m_pSeeking == null)
                    {
                        var _pin = new IPinImpl(m_pPin);
                        if (_pin.IsValid)
                        {
                            IntPtr _connected;
                            var hr = (HRESULT)_pin.ConnectedTo(out _connected);
                            if (SUCCEEDED(hr))
                            {
        ...
        public int GetCapabilities(out AMSeekingSeekingCapabilities pCapabilities)
        {
            pCapabilities = 0;
            var _seeking = Seeking;
            if (_seeking == null || !_seeking.IsValid) return E_NOTIMPL;
        ...
        public int GetCurrentPosition(out long pCurrent)
        {
            pCurrent = 0;
            long lEnd;
            var hr = GetMediaTime(out pCurrent, out lEnd);
            if (SUCCEEDED(hr))
            {
                hr = NOERROR;
            }
        ...
        public int get_Duration(out double pLength)
        {
            pLength = 0;
            long lDuration;
            var hr = GetDuration(out lDuration);
            if (SUCCEEDED(hr)) pLength = lDuration / (double)UNITS;
            return hr;
        }

        public int put_CurrentPosition(double llTime)
        {
            var _position = (long)(llTime * UNITS);
            return SetPositions(_position, AMSeekingSeekingFlags.AbsolutePositioning, 0,
        ...
        public int get_CurrentPosition(out double pllTime)
        {
            pllTime = 0;
            long _position;
            var hr = GetCurrentPosition(out _position);
            if (SUCCEEDED(hr)) pllTime = _position / (double)UNITS;
            return hr;
        }
        ...
        public int get_StopTime(out double pllTime)
        {
            pllTime = 0;
            long _position;
            var hr = GetStopPosition(out _position);
            if (SUCCEEDED(hr)) pllTime = _position / (double)UNITS;
            return hr;
        }
        ...
        {
            var _position = (long)(llTime * UNITS);
            return SetPositions(0, AMSeekingSeekingFlags.NoPositioning, _position,
        ...
        public int get_PrerollTime(out double pllTime)
        {
            pllTime = 0;
            long _position;
            var hr = GetPreroll(out _position);
            if (SUCCEEDED(hr)) pllTime = _position / (double)UNITS;
            return hr;
        }
        ...
        public int CanSeekForward(out OABool pCanSeekForward)
        {
            ...
            var _caps = AMSeekingSeekingCapabilities.CanSeekForwards;
            var hr = CheckCapabilities(ref _caps);
            if (S_OK == hr) pCanSeekForward = OABool.True;
        ...
        public int CanSeekBackward(out OABool pCanSeekBackward)
        {
            ...
            var _caps = AMSeekingSeekingCapabilities.CanSeekBackwards;
            var hr = CheckCapabilities(ref _caps);
            if (S_OK == hr) pCanSeekBackward = OABool.True;
            if (S_FALSE == hr) hr = S_OK;
            return hr;
        }
    ...
    public class RendererPosPassThru : PosPassThru
    {
        ...
        private long m_StartMedia;
        private long m_EndMedia;
        ...
        public int RegisterMediaTime(ref IMediaSampleImpl _sample)
        {
            ...
            lock (m_PositionLock)
            {
                ...
                var hr = _sample.GetTime(out StartMedia, out EndMedia);
                if (FAILED(hr))
                {
        ...
        public override HRESULT GetMediaTime(out long pStartTime, out long pEndTime)
        {
            lock (m_PositionLock)
            {
                pStartTime = 0;
                pEndTime = 0;
                if (m_bReset) return E_FAIL;

                // We don't have to return the end time
                var hr = ConvertTimeFormat(out pStartTime, DsGuid.Empty, m_StartMedia, TimeFormat.MediaTime);
                if (SUCCEEDED(hr)) hr = ConvertTimeFormat(out pEndTime, DsGuid.Empty, m_EndMedia, TimeFormat.MediaTime);
                return (HRESULT)hr;
            }
        }
    ...
    public class RendererInputPin : BaseInputPin
    {
        ...
        public override int BreakConnect()
        {
            var hr = ((BaseRendererFilter)m_Filter).BreakConnect();
            if (FAILED(hr)) return hr;
            return base.BreakConnect();
        }
        ...
        public override int CompleteConnect(ref IPinImpl pReceivePin)
        {
            var hr = (m_Filter as BaseRendererFilter).CompleteConnect(ref pReceivePin);
            if (FAILED(hr)) return hr;
            return base.CompleteConnect(ref pReceivePin);
        }
        ...
        public override int EndOfStream()
        {
            var _filter = m_Filter as BaseRendererFilter;
            lock (_filter.FilterLock)
            {
                lock (_filter.RendererLock)
                {
                    ...
                    if (SUCCEEDED(hr)) hr = base.EndOfStream();
                    return hr;
                }
            }
        }
        ...
        public override int EndFlush()
        {
            ...
            lock (_filter.FilterLock)
            {
                lock (_filter.RendererLock)
                {
                    var hr = _filter.EndFlush();
                    if (SUCCEEDED(hr)) hr = base.EndFlush();
                    return hr;
                }
            }
        }
        ...
        {
            var _filter = m_Filter as BaseRendererFilter;
            var hr = _filter.OnReceive(ref pSample);
            if (FAILED(hr))
                lock (_filter.FilterLock)
                {
                    if (!IsStopped && !IsFlushing && !_filter.IsAbort && !m_bRunTimeError)
                    {
                        ...
                        lock (_filter.RendererLock)
                        {
                            if (_filter.IsStreaming && !_filter.IsEndOfStreamDelivered) _filter.NotifyEndOfStream();
                        }
    ...
    public class MessageDispatcher : IMessageFilter, IDisposable
    {
        ...
        protected EventWaitHandle m_hAbort;
    ...
    public abstract class BaseRendererFilter : BaseFilter, IMediaSeeking
    {
        ...
        protected bool m_bStreaming;
        protected bool m_bEOS;
        protected bool m_bEOSDelivered;
        ...
        protected int m_dwAdvise;
        protected RendererPosPassThru m_pPosition;

        protected IMediaSampleImpl m_pMediaSample;
        protected bool m_bRepaintStatus = true;
        protected long m_SignalTime;

        protected Timer m_EndOfStreamTimer;
        ...
        public RendererInputPin InputPin
        {
            get
            {
                if (Pins.Count >= 1) return (RendererInputPin)Pins[0];
                return null;
            }
        }
        ...
        protected RendererPosPassThru Position
        {
            get
            {
                if (m_pPosition == null)
                    ...
                    {
                        var hr = NOERROR;
                        var _pin = Marshal.GetComInterfaceForObject(InputPin, typeof(IPin));
                        m_pPosition = new RendererPosPassThru(ref hr, _pin);
                        Marshal.Release(_pin);
                        if (FAILED(hr)) m_pPosition = null;
                    }
        ...
        public virtual int Inactive()
        {
            if (Position != null) Position.ResetMediaTime();
        ...
        public virtual int BeginFlush()
        {
            if (m_State == FilterState.Paused) NotReady();
        ...
        public virtual int EndOfStream()
        {
            ...
            m_bEOS = true;
            if (m_pMediaSample != null) return NOERROR;
        ...
        {
            if (m_bEOS == false || m_bEOSDelivered || m_EndOfStreamTimer != null) return NOERROR;

            // If there is no clock then signal immediately
            if (m_pClock == IntPtr.Zero) return NotifyEndOfStream();

            // How long into the future is the delivery time

            var Signal = m_tStart + m_SignalTime;
            long CurrentTime;
            Clock.GetTime(out CurrentTime);
            var Delay = (int)((Signal - CurrentTime) / 10000);
            ...
            m_EndOfStreamTimer = new Timer(TimerCallbackProc);
            if (!m_EndOfStreamTimer.Change(Delay, Timeout.Infinite)) return NotifyEndOfStream();
            return NOERROR;
        }
        ...
        public virtual int BreakConnect()
        {
            ...
            if (InputPin.IsConnected == false) return S_FALSE;

            // Check we are stopped before disconnecting
            if (m_State != FilterState.Stopped && !InputPin.CanReconnectWhenActive) return VFW_E_NOT_STOPPED;

            RepaintStatus = false;
            ResetEndOfStream();
            ClearPendingSample();
            m_bAbort = false;
            if (FilterState.Running == m_State) StopStreaming();
            m_pPosition = null;
            return NOERROR;
        }
        ...
        public virtual int OnReceive(ref IMediaSampleImpl pSample)
        {
            ...
            var hr = PrepareReceive(ref pSample);
            if (hr != S_OK)
            {
                if (hr == VFW_E_SAMPLE_REJECTED) return NOERROR;
                return hr;
            }
        ...
        public virtual int PrepareReceive(ref IMediaSampleImpl pMediaSample)
        {
            lock (m_Lock)
            {
                ...
                var hr = InputPin.BaseReceive(ref pMediaSample);
                ...
                {
                    var mt = (AMMediaType)Marshal.PtrToStructure(InputPin.SampleProps.pMediaType, typeof(AMMediaType));
                ...
                lock (m_RendererLock)
                {
                    ...
                    if (Position != null) Position.RegisterMediaTime(ref pMediaSample);
                    // Schedule the next sample if we are streaming

                    if (m_bStreaming && ScheduleSample(ref pMediaSample) == false)
                    {
                    ...
                    if (m_bStreaming == false) RepaintStatus = true;
                    return NOERROR;
                }
            }
        }
        ...
        public virtual IMediaSampleImpl GetCurrentSample()
        {
            lock (m_RendererLock)
            {
                if (m_pMediaSample != null) m_pMediaSample._AddRef();
        ...
        {
            if (pMediaSample == null || m_bStreaming == false) return S_FALSE;

            OnRenderStart(ref pMediaSample);
            var hr = DoRenderSample(ref pMediaSample);
        ...
        {
            WaitHandle[] WaitObjects = { m_ThreadSignal, m_RenderEvent };
            var Result = WaitHandle.WaitTimeout;

            // Wait for either the time to arrive or for us to be stopped

            OnWaitStart();
            while (Result == WaitHandle.WaitTimeout) Result = WaitHandle.WaitAny(WaitObjects, RENDER_TIMEOUT, false);
            OnWaitEnd();

            // We may have been awoken without the timer firing
            if (Result == 0) return VFW_E_STATE_CHANGED;
            SignalTimerFired();
            return NOERROR;
        }
        ...
        public virtual bool ScheduleSample(ref IMediaSampleImpl pMediaSample)
        {
            ...
            var hr = GetSampleTimes(ref pMediaSample, out StartSample, out EndSample);
            ...
            out long pEndTime)
        {
            ...
            if (m_pClock == IntPtr.Zero) return S_OK;
        ...
        public virtual int StartStreaming()
        {
            lock (m_RendererLock)
            {
                if (m_bStreaming) return NOERROR;
                ...
                ASSERT(m_pMediaSample != null);
                if (!ScheduleSample(ref m_pMediaSample)) m_RenderEvent.Set();
                return NOERROR;
            }
        }
        ...
        public override int Stop()
        {
            lock (m_Lock)
            {
                ...
                if (InputPin.Allocator != null) InputPin.Allocator.Decommit();
        ...
        public override int Pause()
        {
            lock (m_Lock)
            {
                ...
                if (m_State == FilterState.Stopped) m_evAbort.Reset();
                // Make sure there really is a state change

                if (m_State == FilterState.Paused) return CompleteStateChange(FilterState.Paused);
                ...
                if (InputPin.Allocator != null) InputPin.Allocator.Commit();
        ...
        public override int Run(long tStart)
        {
            lock (m_Lock)
            {
                ...
                if (m_State == FilterState.Running) return NOERROR;
        ...
        {
            var hr = base.JoinFilterGraph(pGraph, pName);
            if (FAILED(hr)) return hr;
        ...
        private bool WaitDispatchingMessages(EventWaitHandle hObject, int dwWait, IntPtr hwnd, int uMsg,
            EventWaitHandle hEvent)
        {
            var nCount = null != hEvent ? 2 : 1;

            var hWaitObjects = new WaitHandle[nCount];
            hWaitObjects[0] = hObject;
            if (null != hEvent) hWaitObjects[1] = hEvent;
        ...
        {
            var pSink = (IMediaEventSink)pPin;
            if (pSink != null)
            {
        ...
        public bool OnDisplayChange()
        {
            lock (m_RendererLock)
            {
                if (InputPin.IsConnected == false) return false;
        ...
        public int GetCapabilities(out AMSeekingSeekingCapabilities pCapabilities)
        {
            if (Position != null) return Position.GetCapabilities(out pCapabilities);
            pCapabilities = 0;
            return E_NOINTERFACE;
        }
        ...
        public int CheckCapabilities(ref AMSeekingSeekingCapabilities pCapabilities)
        {
            if (Position != null) return Position.CheckCapabilities(ref pCapabilities);
            return E_NOINTERFACE;
        }

        public int IsFormatSupported(Guid pFormat)
        {
            if (Position != null) return Position.IsFormatSupported(pFormat);
            return E_NOINTERFACE;
        }

        public int QueryPreferredFormat(out Guid pFormat)
        {
            if (Position != null) return Position.QueryPreferredFormat(out pFormat);
            pFormat = Guid.Empty;
            return E_NOINTERFACE;
        }
        ...
        public int GetTimeFormat(out Guid pFormat)
        {
            if (Position != null) return Position.GetTimeFormat(out pFormat);
            pFormat = Guid.Empty;
            return E_NOINTERFACE;
        }
        ...
        public int IsUsingTimeFormat(Guid pFormat)
        {
            if (Position != null) return Position.IsUsingTimeFormat(pFormat);
            return E_NOINTERFACE;
        }

        public int SetTimeFormat(Guid pFormat)
        {
            if (Position != null) return Position.SetTimeFormat(pFormat);
            return E_NOINTERFACE;
        }

        public int GetDuration(out long pDuration)
        {
            if (Position != null) return Position.GetDuration(out pDuration);
            pDuration = 0;
            return E_NOINTERFACE;
        }
        ...
        public int GetStopPosition(out long pStop)
        {
            if (Position != null) return Position.GetStopPosition(out pStop);
            pStop = 0;
            return E_NOINTERFACE;
        }
        ...
        public int GetCurrentPosition(out long pCurrent)
        {
            if (Position != null) return Position.GetCurrentPosition(out pCurrent);
            pCurrent = 0;
            return E_NOINTERFACE;
        }
        ...
        public int ConvertTimeFormat(out long pTarget, DsGuid pTargetFormat, long Source, DsGuid pSourceFormat)
        {
            if (Position != null) return Position.ConvertTimeFormat(out pTarget, pTargetFormat, Source, pSourceFormat);
            pTarget = 0;
            return E_NOINTERFACE;
        }
        ...
        public int SetPositions(DsLong pCurrent, AMSeekingSeekingFlags dwCurrentFlags, DsLong pStop,
            AMSeekingSeekingFlags dwStopFlags)
        {
            if (Position != null) return Position.SetPositions(pCurrent, dwCurrentFlags, pStop, dwStopFlags);
            return E_NOINTERFACE;
        }

        public int GetPositions(out long pCurrent, out long pStop)
        {
            if (Position != null) return Position.GetPositions(out pCurrent, out pStop);
            pCurrent = pStop = 0;
            return E_NOINTERFACE;
        }
        ...
        public int GetAvailable(out long pEarliest, out long pLatest)
        {
            if (Position != null) return Position.GetAvailable(out pEarliest, out pLatest);
            pEarliest = pLatest = 0;
            return E_NOINTERFACE;
        }
        ...
        public int SetRate(double dRate)
        {
            if (Position != null) return Position.SetRate(dRate);
            return E_NOINTERFACE;
        }

        public int GetRate(out double pdRate)
        {
            if (Position != null) return Position.GetRate(out pdRate);
            pdRate = 0;
            return E_NOINTERFACE;
        }
        ...
        public int GetPreroll(out long pllPreroll)
        {
            if (Position != null) return Position.GetPreroll(out pllPreroll);
            pllPreroll = 0;
            return E_NOINTERFACE;
        }
    ...
    public class AsyncStream : Stream
    {
        ...
        protected IAsyncReaderImpl m_Reader;
        protected long m_lPosition;
        protected long m_lLenth;
        ...
        {
            m_Reader = _reader;
            if (m_Reader != null) m_Reader._AddRef();
        }
        ...
        public override long Length
        {
            get
            {
                if (m_lLenth == 0)
                {
                    long lTotal;
                    long lAvailable;
                    if (COMHelper.S_OK == m_Reader.Length(out lTotal, out lAvailable)) m_lLenth = lTotal;
                }
        ...
        {
            var _size = count - offset;
            if (_size <= 0) return 0;
            if (m_lPosition < 0 || m_lPosition >= Length) return 0;
            if (m_Reader != null)
            {
                if (_size > Length - m_lPosition) _size = (int)(Length - m_lPosition);
                var _ptr = IntPtr.Zero;
                try
                {
                    _ptr = Marshal.AllocCoTaskMem(count - offset);
                    if (COMHelper.S_OK == m_Reader.SyncRead(m_lPosition, _size, _ptr))
                    {
        ...
        public override long Seek(long offset, SeekOrigin origin)
        {
            ...
            if (m_lPosition < 0)
                m_lPosition = 0;
            else if (m_lPosition >= _length) m_lPosition = _length;
            return m_lPosition;
        }
    ...
    public class COMStream : Stream
    {
        ...
        protected IStreamImpl m_Reader;
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (m_Reader != null)
            {
                ...
                try
                {
                    ...
                    var _result = buffer;
                    if (offset != 0) _result = new byte[_size];
                    m_Reader.Read(_result, _size, _ptr);
                    var _readed = Marshal.ReadInt32(_ptr);
                    if (_readed > 0 && offset != 0) Array.Copy(_result, 0, buffer, offset, _readed);
                    return _readed;
                }
                finally
                {
    ...
    public class BitStreamReader : COMHelper, IDisposable, IAsyncReader, IStream
    {
        ...
        protected int m_nBitIndex;
        protected int m_nCacheIndex;
        protected byte[] m_pCacheBuffer;
        protected byte[] m_btCurrentByte = new byte[1];
        protected long m_llPosition;
        ...
        public long TotalSize => m_Stream.Length;
        ...
        public bool CanRead => IsAvailable(0);
        ...
        {
            if (_size > 0) return AvailableSize >= _size;
        ...
        public HRESULT Seek(long ullPosition)
        {
            lock (m_csCacheLock)
            {
                if (ullPosition < m_llPosition && m_llPosition - m_nBufferSize < ullPosition)
                {
        ...
        {
            int dwReaded;
            var hr = ReadData(pBuffer, dwSize, out dwReaded);
            if (FAILED(hr)) return hr;
            ...
            lock (m_csCacheLock)
            {
                ...
                while (dwSize > 0)
                {
                    ...
                    if (m_nBufferSize == 0)
                    {
                        ...
                        var _readed = 0;
                        {
                            var hr = ReadData(m_llPosition, m_pCacheBuffer, m_nCacheSize, out _readed);
                    ...
                    var nBytesToWrite =
                        m_nBufferSize - m_nCacheIndex <= dwSize ? m_nBufferSize - m_nCacheIndex : dwSize;
        ...
        {
            return m_nBitIndex == 0;
        }

        public void AlignNextByte()
        {
            if (m_nBitIndex != 0) SkipBits(m_nBitIndex);
        }
        ...
        public int ReadBit()
        {
            ...
            {
                var _readed = 0;
                var hr = ReadData(m_btCurrentByte, 1, out _readed);
        ...
        {
            var llPosition = Position;
            var nBitIndex = m_nBitIndex;
            var _result = ReadBits(nCount);
            Seek(llPosition);
        ...
        public void SkipBits(int nCount)
        {
            while (nCount-- > 0) ReadBit();
        }

        public void SkipBytes(int nCount)
        {
            SkipBits(nCount * 8);
        }
        ...
        public IntPtr ReadValueAlloc(int _size)
        {
            ...
            var _ptr = Marshal.AllocCoTaskMem(_size);
            var _data = new byte[_size];
            if (IsAligned())
            {
                var dwReaded = 0;
                var hr = ReadData(_data, _size, out dwReaded);
            ...
            else
            {
                ...
                {
                    for (var i = 0; i < _size; i++) _data[i] = (byte)ReadByte();
                }
                else
                {
                    Marshal.FreeCoTaskMem(_ptr);
            ...
            if (_ptr != IntPtr.Zero) Marshal.Copy(_data, 0, _ptr, _size);
            return _ptr;
        }
        ...
        {
            var _data = new byte[lLength];
            int _readed;
            var hr = ReadData(llPosition, _data, lLength, out _readed);
            if (_readed > 0 && SUCCEEDED(hr)) Marshal.Copy(_data, 0, pBuffer, _readed);
        ...
        public void Read(byte[] pv, int cb, IntPtr pcbRead)
        {
            ...
            if (pcbRead != IntPtr.Zero) Marshal.WriteInt32(pcbRead, _readed);
        }
        ...
        public void Seek(long dlibMove, int dwOrigin, IntPtr plibNewPosition)
        {
            ...
            if (_position < 0)
                _position = 0;
            else if (_position >= _length) _position = _length;
            Seek(_position);
    ...
    public class PacketsQueue : IDisposable
    {
        ...
        public long Duration
        {
            get
            {
                ...
                {
                    var _duration = m_rtQueueStop - m_rtQueueStart;
        ...
        public long StartTime
        {
            get
            {
                if (m_rtQueueStart == -1) return StopTime;
                return m_rtQueueStart;
            }
        }

        public long StopTime => m_rtQueueStop != -1 ? m_rtQueueStop : 0;
        ...
        public bool IsFull => !m_evNotFull.WaitOne(0, false);
            ...
            if (pPacket != null)
                lock (m_csLock)
                {
                    if (pPacket.Start < 0 || !m_bSortable)
                    {
                        ...
                        {
                            m_rtQueueStop = pPacket.Stop > 0 && pPacket.Stop > pPacket.Start
                                ? pPacket.Stop
                                : pPacket.Start;
                            if (m_Packets.Count == 0) m_rtQueueStart = pPacket.Start;
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var _position = m_Packets.Count - 1;
                                var _skip = _position;
                                while (true)
                                {
                                    var _packet = m_Packets[_position];
                                    if (_packet.Start != -1)
                                    {
        ...
        public bool Peek(out PacketData ppPacket, bool bRemove)
        {
            ...
            var bShouldReset = false;
            {
                lock (m_csLock)
                {
                    ...
                    if (bRemove)
                    {
                        if (m_AddPosition == 0) m_AddPosition = -1;
            ...
            if (bRemove && bShouldReset) m_evNotFull.Set();
            return true;
        }
    ...
    public abstract class DemuxTrack : COMHelper, IDisposable
    {
        ...
        protected FileParser m_pParser;
        protected TrackType m_Type = TrackType.Unknown;
        protected long m_rtOffset;
        protected long m_rtSampleTime;
        protected long m_rtDuration;
        protected long m_rtPosition;
        protected bool m_bActive;
        protected EventWaitHandle m_hFlush;
        protected ManualResetEvent m_evReady = new ManualResetEvent(false);
        protected bool m_bFirstSample = true;
        protected bool m_bInvalidTrack;
        protected string m_sName = "";
        ...
        public bool IsTrackValid => !m_bInvalidTrack;

        public bool IsStartSample => m_bFirstSample;

        public long Offset => m_rtOffset;

        public long Duration
        {
            get
            {
                if (m_rtDuration > 0) return m_rtDuration;
                return m_pParser.Duration;
            }
        ...
        public long Position => m_rtPosition;
        ...
        public long Allocated
        {
            get
            {
                var _time = m_Queue.CacheDuration;
                if (_time > 0) return _time;
                return 0;
            }
        }

        public bool IsWaiting => !m_evReady.WaitOne(0, false) && IsEOS;
        ...
        public bool IsEOS => m_bActive && m_pParser.EOSEvent.WaitOne(0, false) && m_Queue.IsEmpty;
        ...
        public virtual PacketData GetNextPacket()
        {
            ...
            {
                PacketData pPacket;
                if (m_Queue.Peek(out pPacket, true)) return pPacket;

                if (IsEOS) break;
                {
                    if (m_Queue.IsEmpty) m_evReady.Reset();
                }
            }
        ...
        public virtual HRESULT ReadMediaSample(ref IMediaSampleImpl pSample)
        {
            ...
            if (_packet.Start >= 0)
            {
                var _start = _packet.Start;
                var _stop = _packet.Stop;
                if (_start < m_rtOffset)
                {
                    pSample.SetPreroll(true);
                    pSample.SetSyncPoint(false);
                    if (m_Type != TrackType.Audio) pSample.SetDiscontinuity(true);
                }

                if (_stop < 0 || _stop <= _start || _start == _stop + 1)
                    if (m_rtSampleTime > 0)
                        _stop = _start + m_rtSampleTime;
                _start -= m_rtOffset;
                if (_stop > 0)
                {
                    _stop -= m_rtOffset;
        ...
        protected virtual int FillSampleBuffer(IntPtr pBuffer, int _size, ref PacketData _packet)
        {
            ...
            {
                var _buffer = new byte[_packet.Size];
                m_pParser.Stream.ReadData(_packet.Position, _buffer, _packet.Size, out _readed);
                if (_readed > 0) Marshal.Copy(_buffer, 0, pBuffer, _readed);
            }
    ...
    public abstract class FileParser : COMHelper, IDisposable
    {
        ...
        protected BitStreamReader m_Stream;
        protected List<DemuxTrack> m_Tracks = new List<DemuxTrack>();
        protected long m_rtDuration;
        protected EventWaitHandle m_hQuit;
        protected EventWaitHandle m_hEOS;
        ...
        {
            RequireDemuxThread = bRequireDemuxThread;
        }
        ...
        public DemuxTrack this[int iIndex] => m_Tracks[iIndex];

        public bool RequireDemuxThread { get; set; }
        ...
        public long Duration
        {
            ...
            {
                if (m_rtDuration == 0)
                    foreach (var _track in m_Tracks)
                    {
                        var _duration = _track.TrackDuration + _track.Offset;
                        if (m_rtDuration < _duration) m_rtDuration = _duration;
                    }
        ...
        public long Position
        {
            get
            {
                ...
                foreach (var _track in m_Tracks)
                    if (_track.Active)
                    {
                        var _position = _track.Position;
                        if (_position > 0 && _position < _time) _time = _position;
                    }
        ...
        public BitStreamReader Stream => m_Stream;
        ...
        public virtual HRESULT OpenInput(BitStreamReader _stream)
        {
            ...
            m_Stream = _stream;
            var hr = CheckFile();
            if (hr == S_OK) hr = LoadTracks();
            if (hr != S_OK) CloseInput();
            return hr;
        }
        ...
        {
            foreach (var _track in m_Tracks) _track.SeekTrack(_time);
            return NOERROR;
        }
    ...
    public class SplitterOutputPin : BaseOutputPin, IMediaSeeking
    {
        ...
        protected virtual void ThreadProc()
        {
            ...
            while (!m_evQuit.WaitOne(0, false))
            {
                ...
                if (!bEOS)
                    lock (m_csThreadLock)
                    {
                        ...
                        if (hr.Succeeded)
                        {
                            ...
                            {
                                if (FAILED(SetMediaType(pmt))) _sample.SetMediaType(null);
                                pmt.Free();
                            }
                            ...
                            else
                            {
                                if (S_OK == _sample.GetTime(out tStart, out tStop))
                                {
                                    ...
                                    if (tStart < 0) _sample.SetPreroll(true);
                ...
                if (bEOS)
                {
                    ...
                    if (!m_evFlush.WaitOne(0, false) && !(m_Filter as BaseSplitter).QuitEvent.WaitOne(0, false) &&
                        !m_evQuit.WaitOne(0, false))
                    {
                        DeliverBeginFlush();
                        DeliverEndFlush();
                        TRACE("EOS Delivered");
        ...
        protected bool m_bHaveException;
        protected AMThread m_WorkerThread;
            ...
            : base(_name, _filter)
        {
            ...
            m_WorkerThread = new ManagedThread(ThreadProc);
        }
        ...
        public bool EOSDelivered => m_bEOSDelivered;

        public DemuxTrack Track => m_pTrack;

        public bool StreamEnabled
        {
            get => m_bStreamEnabled;
            set
            {
                lock (m_csEnableLock)
                {
                    if (m_bStreamEnabled != value) m_bStreamEnabled = value;
                }
            }
        }
        ...
        public override int Active()
        {
            ...
            var hr = base.Active();
            if (SUCCEEDED(hr) && IsConnected) StartThread();
            return hr;
        }
        ...
        public override int SetMediaType(AMMediaType mt)
        {
            var hr = m_pTrack.SetMediaType(mt);
            if (FAILED(hr)) return hr;
        ...
        public override int DecideBufferSize(ref IMemAllocatorImpl pAlloc, ref AllocatorProperties prop)
        {
            ...
            if (m_mt.majorType == MediaType.Audio)
            {
                ...
                if (prop.cbBuffer == 0) prop.cbBuffer = (_wfx.nSamplesPerSec * _wfx.nChannels) << 3;
            }

            {
                var lBufferSize = prop.cbBuffer;
                var wBuffers = (short)prop.cBuffers;
            ...
            if (prop.cbAlign < 1) prop.cbAlign = 1;
            prop.cbPrefix = 0;

            if (prop.cbBuffer == 0) return E_UNEXPECTED;

            var _actual = new AllocatorProperties();
        ...
        {
            m_evQuit.Reset();
            if (m_evReady.WaitOne(0, false)) m_WorkerThread.Create();
        }
        ...
        public virtual int CheckCapabilities(ref AMSeekingSeekingCapabilities pCapabilities)
        {
            ...
            if (((int)pCapabilities & ~(int)dwActual) == 0) return S_FALSE;
            return S_OK;
        }

        public virtual int IsFormatSupported(Guid pFormat)
        {
            if (pFormat == TimeFormat.MediaTime) return S_OK;
            return S_FALSE;
        }
        ...
        {
            Guid guidActual;
            var hr = GetTimeFormat(out guidActual);
        ...
        {
            pDuration = 0;
            if (m_pTrack != null) pDuration = m_pTrack.Duration + m_pTrack.Offset;
            return NOERROR;
        }
        ...
        public virtual int SetPositions(DsLong pCurrent, AMSeekingSeekingFlags dwCurrentFlags, DsLong pStop,
            AMSeekingSeekingFlags dwStopFlags)
        {
            var _filter = m_Filter as BaseSplitter;
            if (_filter.SelectSeekingPin(this))
            {
                ...
                {
                    if (((int)dwStopFlags & (int)AMSeekingSeekingFlags.RelativePositioning) != 0) _stop += pStop;
                }

                if (((int)dwCurrentFlags & (int)AMSeekingSeekingFlags.PositioningBitsMask) != 0)
                    return _filter.SetPositions(_start, _stop, -1);
                if (((int)dwStopFlags & (int)AMSeekingSeekingFlags.PositioningBitsMask) != 0)
                    // stop change only
        ...
        public virtual int GetPositions(out long pCurrent, out long pStop)
        {
            ...
            if (m_rtPosition >= 0) pCurrent = m_rtPosition;
            return NOERROR;
        }
    ...
    public abstract class BaseSplitter : BaseFilter, IMediaSeeking, IAMStreamSelect
    {
        ...
        protected long m_tStartTime;
        protected long m_tStopTime = MAX_LONG;
        protected double m_dRate = 1.0;
        protected SplitterOutputPin m_pSeekingPin;
        protected ManualResetEvent m_evQuit = new ManualResetEvent(false);
        protected ManualResetEvent m_evReady = new ManualResetEvent(true);
        ...
        protected object m_csThreadLock = new object();
        protected FileParser m_pFileParser;
        protected AMThread m_WorkerThread;
            ...
            get
            {
                if (string.IsNullOrEmpty(m_sFileName) && InputPin != null)
                    try
                    {
                        ...
                        {
                            var _source = (IFileSourceFilter)_info.filter;
        ...
        public EventWaitHandle QuitEvent => m_evQuit;

        public SplitterInputPin InputPin
        {
            get
            {
                if (Pins.Count > 0 && Pins[0].Direction == PinDirection.Input) return Pins[0] as SplitterInputPin;
                return null;
            }
        }
        ...
        public SplitterOutputPin SeekingPin
        {
            get
            {
                lock (m_csSeeking)
                {
                    if (m_pSeekingPin != null && !m_pSeekingPin.IsConnected) m_pSeekingPin = null;
                    if (m_pSeekingPin == null)
                    {
                        foreach (var _pin in Pins)
                            ...
                            if (_pin.Direction == PinDirection.Output)
                            {
                                ...
                                else
                                {
                                    if (_pin.IsConnected && _pin != m_pSeekingPin)
                                        if (m_pSeekingPin.Track.TrackDuration <
                                            (_pin as SplitterOutputPin).Track.TrackDuration)
                                            m_pSeekingPin = _pin as SplitterOutputPin;
                                }
                            }
                    }
        ...
        protected override int AfterInstall(HRESULT hr, ref RegFilter2 _reginfo, ref IFilterMapper2 _mapper2)
        {
            if (hr.Succeeded)
            {
                ...
                foreach (var _parser in m_Parsers)
                {
        ...
        public virtual HRESULT CompleteConnect(ref IPinImpl pReceivePin)
        {
            ...
            var _guid = typeof(IAsyncReader).GUID;
            var pAsyncReader = IntPtr.Zero;
            ...
            foreach (var _parser in m_Parsers)
            {
                ...
                if (S_OK == _parser.OpenInput(sFileName))
                {
                    if (_parser.Count > 0)
                    {
                        ...
                        if (InitializeOutputPins() == S_OK) return S_OK;
        ...
        protected virtual HRESULT InitializeOutputPins()
        {
            ...
            var nAdded = 0;
            var nCount = m_pFileParser.Count;
            var _indexes = new int[(int)DemuxTrack.TrackType.Subtitles + 1];
            var _useIndexes = new bool[_indexes.Length];
            ...
            for (var i = 0; i < nCount; i++)
            {
                ...
                if (_track != null && _track.IsTrackValid)
                {
                    ...
                    if (string.IsNullOrEmpty(_name))
                    {
                        ...
                        var _type = (int)_track.Type;
                        if (_type >= 0 && _type < _indexes.Length)
                            if (_useIndexes[_type])
                                _name += " " + _indexes[_type]++;
                    }

                    if (!_track.Enabled) _name = "~" + _name;
        ...
        {
            var _attributes = Attribute.GetCustomAttributes(_type, typeof(RegisterProtocolExtension));
            if (_attributes != null)
                foreach (RegisterProtocolExtension _setup in _attributes)
                    if (_setup != null && !string.IsNullOrEmpty(_setup.Protocol))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                _key = Registry.ClassesRoot.CreateSubKey(_setup.Protocol + "\\Extensions");
                                foreach (var _extension in _setup.Extensions)
                                    if (!string.IsNullOrEmpty(_extension))
                                        _key.SetValue(_extension, GetType().GUID.ToString("B"));
                            }
                            else
                            {
                                _key = Registry.ClassesRoot.CreateSubKey(_setup.Protocol);
                                _key.SetValue("Source Filter", GetType().GUID.ToString("B"));
                            }
                        }
                        catch (Exception _exception)
                        {
        ...
        {
            var _attributes = Attribute.GetCustomAttributes(_type, typeof(RegisterFileExtension));
            if (_attributes != null)
                foreach (RegisterFileExtension _setup in _attributes)
                    if (_setup != null && !string.IsNullOrEmpty(_setup.Extension))
                    {
                        ...
                        try
                        {
                            _key = Registry.ClassesRoot.CreateSubKey("Media Type\\Extensions\\" + _setup.Extension);
                            _key.SetValue("Source Filter", GetType().GUID.ToString("B"));
                            if (_setup.MediaType != Guid.Empty)
                                _key.SetValue("Media Type", _setup.MediaType.ToString("B"));
                            if (_setup.SubType != Guid.Empty) _key.SetValue("SubType", _setup.SubType.ToString("B"));
                        }
                        catch (Exception _exception)
                        {
                            var hr = (HRESULT)Marshal.GetHRForException(_exception);
        ...
        {
            var _attributes = Attribute.GetCustomAttributes(_type, typeof(RegisterMediaType));
            if (_attributes != null)
                ...
                foreach (RegisterProtocolExtension _setup in _attributes)
                    if (_setup != null && !string.IsNullOrEmpty(_setup.Protocol))
                    {
                        ...
                        try
                        {
                            if (_setup.Extensions.Count > 0)
                            {
                                _key = Registry.ClassesRoot.OpenSubKey(_setup.Protocol + "\\Extensions");
                                if (_key != null)
                                {
                                    foreach (var _extension in _setup.Extensions)
                                        if (!string.IsNullOrEmpty(_extension))
                                    ...
                                    if (_key.ValueCount == 0 && _key.SubKeyCount == 0)
                                    {
                                        _key.Close();
                                        _key = null;
                                        _key = Registry.ClassesRoot.OpenSubKey(_setup.Protocol);
                                        _key.DeleteSubKeyTree("Extensions");
                                        if (_key.ValueCount == 0 && _key.SubKeyCount == 0)
                                        {
                                            _key.Close();
                                            _key = null;
                                            Registry.ClassesRoot.DeleteSubKeyTree(_setup.Protocol);
                                        }
                                    }
                                }
                            }
            ...
            if (_attributes != null)
                foreach (RegisterFileExtension _setup in _attributes)
                    if (_setup != null && !string.IsNullOrEmpty(_setup.Extension))
                    {
                        ...
                        try
                        {
                            _key = Registry.ClassesRoot.OpenSubKey("Media Type\\Extensions\\" + _setup.Extension);
                            if (_key != null)
                            {
                                var _guid = (string)_key.GetValue("Source Filter", "");
                                if (_guid == GetType().GUID.ToString("B"))
                                {
                                    _key.Close();
                                    _key = Registry.ClassesRoot.OpenSubKey("Media Type\\Extensions");
                    ...
                    if (_setup != null && !string.IsNullOrEmpty(_setup.Sequence))
                    {
                        ...
                        try
                        {
                            ...
                            if (_setup.MajorType != Guid.Empty)
                                ...
                                _path += MediaType.Stream.ToString("B");
                            _key = Registry.ClassesRoot.OpenSubKey(_path);
                            if (_key != null) _key.DeleteSubKeyTree(_setup.SubType.ToString("B"));
                        }
                        catch (Exception _exception)
                        {
                            var hr = (HRESULT)Marshal.GetHRForException(_exception);
        ...
        public bool SelectSeekingPin(SplitterOutputPin pPin)
        {
            lock (m_csSeeking)
            {
                if (m_pSeekingPin == null) m_pSeekingPin = pPin;
        ...
        public void DeselectSeekingPin(SplitterOutputPin pPin)
        {
            lock (m_csSeeking)
            {
                if (pPin == m_pSeekingPin) m_pSeekingPin = null;
            }
        }
            ...
            var _seek = false;
            {
                if (_start != -1)
                {
                    if (SeekingPin != null)
                    {
                        ...
                        if (_position != _start) _seek = true;
                    }
                    else
                    {
                ...
                if (_stop != -1)
                {
                    if (m_tStopTime != _stop) _seek = true;
                }
                else
                {
                    _stop = m_tStopTime;
                ...
                if (m_pFileParser.RequireDemuxThread)
                {
                    ...
                    lock (m_csThreadLock)
                    {
                        if (m_evReady.WaitOne(0, false))
                        {
                            ...
                            var nResult = WaitHandle.WaitAny(new WaitHandle[] { m_evDemuxStarted, m_evReady });
                            if (nResult != 0)
                            {
                                m_evQuit.Set();
                                m_WorkerThread.Close();
            ...
            if (hr.Succeeded)
            {
                ...
                if (!m_evQuit.WaitOne(0, false))
                    while (!m_evQuit.WaitOne(0, false))
                    {
                        var _duration = UNITS;
                        var nCount = 0;
                        foreach (var pPin in Pins)
                            if (pPin.Direction == PinDirection.Output)
                            {
                                var _pin = pPin as SplitterOutputPin;
        ...
        public int Load(string pszFileName, AMMediaType pmt)
        {
            ...
            try
            {
                _stream = new BitStreamReader(new FileStream(m_sFileName, FileMode.Open, FileAccess.Read,
                    FileShare.ReadWrite));
                if (S_OK == DecideFileParser(_stream))
                {
                    m_bLoading = false;
        ...
        public int QueryProgress(out long pllTotal, out long pllCurrent)
        {
            ...
            if (m_pFileParser != null) return m_pFileParser.GetOpeningProgress(out pllTotal, out pllCurrent);
            return NOERROR;
        }
        ...
        public int GetCapabilities(out AMSeekingSeekingCapabilities pCapabilities)
        {
            ...
            if (SeekingPin != null) return SeekingPin.GetCapabilities(out pCapabilities);
            return E_NOINTERFACE;
        }

        public int CheckCapabilities(ref AMSeekingSeekingCapabilities pCapabilities)
        {
            if (SeekingPin != null) return SeekingPin.CheckCapabilities(ref pCapabilities);
            return E_NOINTERFACE;
        }

        public int IsFormatSupported(Guid pFormat)
        {
            if (SeekingPin != null) return SeekingPin.IsFormatSupported(pFormat);
            return E_NOINTERFACE;
        }
        ...
        public int QueryPreferredFormat(out Guid pFormat)
        {
            ...
            if (SeekingPin != null) return SeekingPin.QueryPreferredFormat(out pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            pFormat = TimeFormat.None;
            if (SeekingPin != null) return SeekingPin.GetTimeFormat(out pFormat);
            return E_NOINTERFACE;
        }

        public int IsUsingTimeFormat(Guid pFormat)
        {
            if (SeekingPin != null) return SeekingPin.IsUsingTimeFormat(pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            pStop = 0;
            if (SeekingPin != null) return SeekingPin.GetStopPosition(out pStop);
            return E_NOINTERFACE;
        }
        ...
        {
            pCurrent = 0;
            if (SeekingPin != null) return SeekingPin.GetCurrentPosition(out pCurrent);
            return E_NOINTERFACE;
        }
        ...
        public int SetPositions(DsLong pCurrent, AMSeekingSeekingFlags dwCurrentFlags, DsLong pStop,
            AMSeekingSeekingFlags dwStopFlags)
        {
            if (SeekingPin != null) return SeekingPin.SetPositions(pCurrent, dwCurrentFlags, pStop, dwStopFlags);
            return E_NOINTERFACE;
        }
        ...
        public int GetPositions(out long pCurrent, out long pStop)
        {
            pCurrent = 0;
            pStop = 0;
            if (SeekingPin != null) return SeekingPin.GetPositions(out pCurrent, out pStop);
            return E_NOINTERFACE;
        }
        ...
        public int GetAvailable(out long pEarliest, out long pLatest)
        {
            pEarliest = 0;
            pLatest = 0;
            if (SeekingPin != null) return SeekingPin.GetAvailable(out pEarliest, out pLatest);
            return E_NOINTERFACE;
        }

        public int SetRate(double dRate)
        {
            if (SeekingPin != null) return SeekingPin.SetRate(dRate);
            return E_NOINTERFACE;
        }
        ...
        {
            pdRate = 1.0;
            if (SeekingPin != null) return SeekingPin.GetRate(out pdRate);
            return E_NOINTERFACE;
        }
        ...
        {
            pllPreroll = 0;
            if (SeekingPin != null) return SeekingPin.GetPreroll(out pllPreroll);
            return E_NOINTERFACE;
        }
        ...
        public int Info(int lIndex, IntPtr ppmt, IntPtr pdwFlags, IntPtr plcid, IntPtr pdwGroup, IntPtr ppszName,
            IntPtr ppObject, IntPtr ppUnk)
        {
            ...
            if (ppmt != IntPtr.Zero)
            {
                ...
                {
                    if (S_OK != _selected.GetMediaType(0, ref mt)) bHaveType = false;
                }

                if (bHaveType)
                {
                    var _pmt = Marshal.AllocCoTaskMem(Marshal.SizeOf(mt));
                    Marshal.StructureToPtr(mt, _pmt, true);
            ...
            {
                var _lcid = _selected.Track.LCID;
                if (_lcid == 0) _lcid = LOCALE_NEUTRAL;
                Marshal.WriteInt32(plcid, _lcid);
            }

            if (pdwGroup != IntPtr.Zero) Marshal.WriteInt32(pdwGroup, 0);
            if (ppszName != IntPtr.Zero)
            {
                var _name = _selected.Track.Name;
                if (string.IsNullOrEmpty(_name)) _name = "Audio #" + lIndex;
                Marshal.WriteIntPtr(ppszName, Marshal.StringToCoTaskMemUni(_name));
            }

            if (ppObject != IntPtr.Zero) Marshal.WriteIntPtr(ppObject, Marshal.GetIUnknownForObject(_selected));
            if (ppUnk != IntPtr.Zero) Marshal.WriteIntPtr(ppUnk, IntPtr.Zero);
            return NOERROR;
        }
        ...
        public int Enable(int lIndex, AMSTREAMSELECTENABLEFLAGS dwFlags)
        {
            ...
            if (IsActive && dwFlags != AMSTREAMSELECTENABLEFLAGS.DISABLE)
                try
                {
                    var _seeking = (IMediaSeeking)FilterGraph;
                    if (_seeking != null)
                    {
                        long _current;
                        _seeking.GetCurrentPosition(out _current);
                        _current -= UNITS / 10;
                        _seeking.SetPositions(_current, AMSeekingSeekingFlags.AbsolutePositioning, null,
                            AMSeekingSeekingFlags.NoPositioning);
                        _current += UNITS / 10;
                        _seeking.SetPositions(_current, AMSeekingSeekingFlags.AbsolutePositioning, null,
                            AMSeekingSeekingFlags.NoPositioning);
    ...
    public class BaseSplitterFilter : BaseSplitter
    {
        ...
        protected BaseSplitterFilter(string _name, FileParser _parser)
            : base(_name)
        {
            if (_parser != null) m_Parsers.Add(_parser);
    ...
    public abstract class MuxerTrack : COMHelper, IDisposable
    {
        ...
        protected PacketsQueue m_Queue = new PacketsQueue();
        protected FileWriter m_pWriter;
        protected TrackType m_Type = TrackType.Unknown;
        protected long m_rtPosition;
        protected EventWaitHandle m_hFlush;
        protected ManualResetEvent m_evReady = new ManualResetEvent(false);
        ...
        public bool IsWaiting => !m_evReady.WaitOne(0, false) && m_Queue.IsEmpty;
    ...
    public abstract class FileWriter : COMHelper, IDisposable
    {
        ...
        {
            CloseOutput();
            while (m_Tracks.Count > 0) RemoveTrack(m_Tracks[0]);
        }
        ...
        protected BitStreamWriter m_Stream;
        protected List<MuxerTrack> m_Tracks = new List<MuxerTrack>();
        protected string m_sFileName = "";
        protected EventWaitHandle m_hQuit;
        protected bool m_bThreadExists;
        protected bool m_bRealtimeSync;
        ...
        {
            RequireMuxThread = bRequireMuxThread;
        }
        ...
        public bool RequireMuxThread { get; set; }
        ...
        public virtual HRESULT OpenOutput(string sFileName, bool bOverwrite)
        {
            ...
            var hr = OpenFile();
            if (hr == S_OK) return hr;
            var _stream = new FileStream(sFileName, bOverwrite ? FileMode.Create : FileMode.OpenOrCreate,
        ...
        {
            if (pmt == null) return E_POINTER;
            var _track = CreateTrackForType(pmt);
            if (_track == null) return VFW_E_CANNOT_CONNECT;
            var hr = _track.InitTrack();
            RemoveTrack(_track);
            return hr;
        }
        ...
        {
            var _track = CreateTrack(pmt);
            if (_track != null) m_Tracks.Add(_track);
            return _track;
        }
        ...
        public virtual HRESULT RemoveTrack(MuxerTrack _track)
        {
            ...
            {
                _track.Dispose();
                if (m_Tracks.Remove(_track)) return S_OK;
            }
        ...
        public virtual HRESULT OnStartWriting()
        {
            foreach (var _track in m_Tracks) _track.OnStartWriting();
            return NOERROR;
        }

        public virtual HRESULT OnStopWriting()
        {
            foreach (var _track in m_Tracks) _track.OnStopWriting();
            return NOERROR;
        }
    ...
    public class MuxerInputPin : BaseInputPin
    {
        ...
        protected MuxerTrack m_pTrack;
        protected bool m_bEOSDelivered;
        protected RendererPosPassThru m_pPosition;
        protected long m_rtLastSampleTime = -1;
        protected long m_rtStartPosition;
        protected long m_rtWritingPosition;
        protected long m_lSampleSize;
        protected ManualResetEvent m_evFlush = new ManualResetEvent(false);
        protected ManualResetEvent m_evRenderTime = new ManualResetEvent(true);
        protected IReferenceClockImpl m_pClock;
        ...
        public RendererPosPassThru Position
        {
            get
            {
                if (m_pPosition == null)
                    ...
                    {
                        var hr = NOERROR;
                        var _pin = Marshal.GetComInterfaceForObject(this, typeof(IPin));
                        m_pPosition = new RendererPosPassThru(ref hr, _pin);
                        Marshal.Release(_pin);
                        if (FAILED(hr)) m_pPosition = null;
                    }
        ...
        public MuxerTrack Track => m_pTrack;
        ...
        {
            var _position = Position;
            if (_position != null) _position.ResetMediaTime();
            m_evFlush.Reset();
            m_bEOSDelivered = false;
            if (m_pTrack != null)
            {
        ...
        public override int Inactive()
        {
            ...
            m_bEOSDelivered = true;
            m_evRenderTime.Set();
            if (m_pTrack != null) m_pTrack.Flush();
            if (m_pClock != null)
            {
                m_pClock._Release();
                m_pClock = null;
        ...
        {
            m_bEOSDelivered = true;
            var hr = base.EndOfStream();
            var _position = Position;
            if (_position != null) _position.EOS();
            lock (m_csTrackLock)
            {
                m_pTrack.IsEOS = true;
            }
        ...
        public override int CheckMediaType(AMMediaType pmt)
        {
            if (S_OK != (m_Filter as BaseMuxer).Writer.CheckInputType(pmt)) return VFW_E_INVALIDMEDIATYPE;
            return NOERROR;
        }

        public override int CompleteConnect(ref IPinImpl pReceivePin)
        {
            var hr = (HRESULT)base.CompleteConnect(ref pReceivePin);
            if (hr.Failed) return hr;
            lock (m_csTrackLock)
            {
                if (m_pTrack != null) (m_Filter as BaseMuxer).Writer.RemoveTrack(m_pTrack);
        ...
        public override int BreakConnect()
        {
            var hr = (HRESULT)base.BreakConnect();
            lock (m_csTrackLock)
            {
                (m_Filter as BaseMuxer).Writer.RemoveTrack(m_pTrack);
                m_pTrack = null;
        ...
        public override int BeginFlush()
        {
            ...
            m_evRenderTime.Set();
            m_evFlush.Set();
            if ((m_Filter as BaseMuxer).SeekingPin == this) (m_Filter as BaseMuxer).BeginFlush();
            return hr;
        }
        ...
        public override int EndFlush()
        {
            ...
            if ((m_Filter as BaseMuxer).SeekingPin == this) (m_Filter as BaseMuxer).EndFlush();
            lock (m_csTrackLock)
            {
                m_pTrack.Flush();
                m_rtStartPosition = 0;
        ...
        public override int OnReceive(ref IMediaSampleImpl _sample)
        {
            ...
            if (hr == S_OK && !m_bEOSDelivered && !m_evFlush.WaitOne(0, false))
            {
                ...
                var lSize = _sample.GetActualDataLength();
                if (hr != S_OK)
                {
                    if (hr != VFW_S_NO_STOP_TIME) _start = m_rtStartPosition;
                    if (m_mt.majorType == MediaType.Audio)
                    {
                        if (m_lSampleSize != 0)
                ...
                if (m_pClock != null)
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        var dwResult = WaitHandle.WaitAny(new WaitHandle[]
                            { m_evRenderTime, (m_Filter as BaseMuxer).Writer.QuitEvent });
                        m_pClock.Unadvise(dwAdvise);
                        if (dwResult != 0) return S_FALSE;
                    }
                }

                if (m_pPosition != null) m_pPosition.RegisterMediaTime(_start, _stop);
                lock (m_csTrackLock)
                {
    ...
    public class MuxerOutputPin : BaseOutputPin
    {
        ...
        public override int DecideBufferSize(ref IMemAllocatorImpl pAlloc, ref AllocatorProperties prop)
        {
            ...
            foreach (var _pin in m_Filter.Pins)
                if (_pin.Direction == PinDirection.Input && _pin.IsConnected)
                {
                    var _allocated = new AllocatorProperties();
    ...
    public abstract class BaseMuxer : BaseFilter, IMediaSeeking, IAMFilterMiscFlags
    {
        ...
        protected FileWriter m_pFileWriter;
        protected string m_sFileName = "";
        protected bool m_bOverwrite = true;
        protected MuxerInputPin m_pSeekingPin;
        ...
        public FileWriter Writer => m_pFileWriter;
        ...
        public bool ThreadExists => m_WorkerThread.ThreadExists;

        public long StartTime => m_tStart;
        ...
        {
            var _pin = OutputPin;
            if (_pin != null) _pin.DeliverBeginFlush();
        ...
        {
            var _pin = OutputPin;
            if (_pin != null) _pin.DeliverEndFlush();
            if (!m_pFileWriter.AppendOnSeek && !m_bOverwrite)
            {
                m_pFileWriter.ThreadExists = false;
                HRESULT hr;
        ...
        public virtual HRESULT EndOfStream()
        {
            ...
            {
                var _pin = OutputPin;
                if (_pin != null) _pin.DeliverEndOfStream();
                StopThread();
                CloseFile();
                NotifyEvent(EventCode.Complete, (IntPtr)(int)S_OK, Marshal.GetIUnknownForObject(this));
            }
        ...
        public virtual HRESULT SetMediaType(AMMediaType pmt)
        {
            if (m_pFileWriter != null) return m_pFileWriter.SetMediaType(pmt);
            return NOERROR;
        }

        public virtual HRESULT GetMediaType(int iPosition, ref AMMediaType pmt)
        {
            if (m_pFileWriter != null) return m_pFileWriter.GetMediaType(iPosition, ref pmt);
            return E_NOTIMPL;
        }
        ...
        public virtual HRESULT CompleteConnect(MuxerInputPin _pin)
        {
            var nConnected = 0;
            var nCount = 0;
            foreach (var _existingPin in Pins)
                if (_existingPin.Direction == PinDirection.Input)
                {
                    nCount++;
                    if (_existingPin.IsConnected) nConnected++;
                }

            if (nCount == nConnected)
            {
                var _max = m_pFileWriter.GetMaxTrackCount();
                if (_max > 0)
                    if (nCount >= _max)
                        return NOERROR;
                AddPin(CreateNewInputPin());
        ...
        protected virtual MuxerInputPin CreateNewInputPin()
        {
            ...
            foreach (var _pin in Pins)
                if (_pin.Direction == PinDirection.Input)
                    nIndex++;
            return new MuxerInputPin("Input " + nIndex, this);
        }
        ...
        protected HRESULT OpenFile()
        {
            if (!m_bIsOpened)
            {
                ...
                if (hr.Failed)
                {
                    var _output = OutputPin;
                    if (_output != null)
                    {
                        var _guid = typeof(IStream).GUID;
                        IntPtr pStream;
                        hr = _output.Connected._QueryInterface(ref _guid, out pStream);
                        if (hr.Succeeded)
                        {
                            var _writer = new BitStreamWriter(new COMStream(pStream));
                            try
                            {
                            ...
                            if (hr != S_OK) m_pFileWriter.CloseOutput();
                        }
                    }
                }
            ...
            if (m_pFileWriter.RequireMuxThread)
            {
                ...
                lock (m_csThreadLock)
                {
                    if (m_evReady.WaitOne(0, false))
                    {
                        ...
                        m_evReady.Reset();
                        m_WorkerThread.Create();
                        var nResult = WaitHandle.WaitAny(new WaitHandle[] { m_evMuxStarted, m_evReady });
                        if (nResult != 0)
                        {
                            m_evQuit.Set();
                            m_WorkerThread.Close();
                    ...
                    if (!bFlush)
                    {
                        ...
                        foreach (var _pin in Pins)
                            if (_pin.Direction == PinDirection.Input && _pin.IsConnected)
                                if (!(_pin as MuxerInputPin).EOSDelivered)
                                {
                                    var _track = (_pin as MuxerInputPin).Track;
                                    if (_minPosition > _track.Position) _minPosition = _track.Position;
                                    if (_track.Position > _maxPosition) _maxPosition = _track.Position;
                                }
                    }
                    else
                    {
                            ...
                            if (_pin.Direction == PinDirection.Input && _pin.IsConnected)
                                if (!(_pin as MuxerInputPin).EOSDelivered)
                                {
                                    ...
                                    if (_track.Position >= _minPosition)
                                        while (true)
                                        {
                                            var _packet = _track.GetNextPacket();
        ...
        {
            pCapabilities = AMSeekingSeekingCapabilities.None;
            if (SeekingPin != null) return SeekingPin.Position.GetCapabilities(out pCapabilities);
            return E_NOINTERFACE;
        }

        public int CheckCapabilities(ref AMSeekingSeekingCapabilities pCapabilities)
        {
            if (SeekingPin != null) return SeekingPin.Position.CheckCapabilities(ref pCapabilities);
            return E_NOINTERFACE;
        }

        public int IsFormatSupported(Guid pFormat)
        {
            if (SeekingPin != null) return SeekingPin.Position.IsFormatSupported(pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            pFormat = TimeFormat.None;
            if (SeekingPin != null) return SeekingPin.Position.QueryPreferredFormat(out pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            pFormat = TimeFormat.None;
            if (SeekingPin != null) return SeekingPin.Position.GetTimeFormat(out pFormat);
            return E_NOINTERFACE;
        }

        public int IsUsingTimeFormat(Guid pFormat)
        {
            if (SeekingPin != null) return SeekingPin.Position.IsUsingTimeFormat(pFormat);
            return E_NOINTERFACE;
        }
        ...
        {
            pStop = 0;
            if (SeekingPin != null) return SeekingPin.Position.GetStopPosition(out pStop);
            return E_NOINTERFACE;
        }
        ...
        {
            pCurrent = 0;
            if (SeekingPin != null) return SeekingPin.Position.GetCurrentPosition(out pCurrent);
            return E_NOINTERFACE;
        }
        ...
        public int GetPositions(out long pCurrent, out long pStop)
        {
            pCurrent = 0;
            pStop = 0;
            if (SeekingPin != null) return SeekingPin.Position.GetPositions(out pCurrent, out pStop);
            return E_NOINTERFACE;
        }
        ...
        public int GetAvailable(out long pEarliest, out long pLatest)
        {
            pEarliest = 0;
            pLatest = 0;
            if (SeekingPin != null) return SeekingPin.Position.GetAvailable(out pEarliest, out pLatest);
            return E_NOINTERFACE;
        }

        public int SetRate(double dRate)
        {
            if (SeekingPin != null) return SeekingPin.Position.SetRate(dRate);
            return E_NOINTERFACE;
        }
        ...
        {
            pdRate = 1.0;
            if (SeekingPin != null) return SeekingPin.Position.GetRate(out pdRate);
            return E_NOINTERFACE;
        }
        ...
        {
            pllPreroll = 0;
            if (SeekingPin != null) return SeekingPin.Position.GetPreroll(out pllPreroll);
            return E_NOINTERFACE;
        }
