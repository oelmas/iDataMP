    ...
    public static class Common
    {
        ...
        public static GMapMarker getMAVMarker(MAVState MAV, GMapOverlay overlay = null)
        {
            ...
            if (overlay != null)
            {
                ...
                if (existing.Count() > 1) existing.Skip(1).ToArray().ForEach(a => { overlay.Markers.Remove(a); });
                if (existing.Count() > 0)
                {
            ...
            if (MAV.aptype == MAVLink.MAV_TYPE.FIXED_WING || (MAV.aptype >= MAVLink.MAV_TYPE.VTOL_DUOROTOR &&
            ...
            if (MAV.aptype == MAVLink.MAV_TYPE.COAXIAL)
                return new GMapMarkerSingle(portlocation, MAV.cs.yaw,
                        MAV.cs.groundcourse, MAV.cs.nav_bearing, MAV.sysid)
                    { Tag = MAV };

            if (MAV.cs.firmware == Firmwares.ArduCopter2 || MAV.aptype == MAVLink.MAV_TYPE.QUADROTOR)
            {
        ...
        public static Form LoadingBox(string title, string promptText)
        {
            ...
            var label = new Label();
            var resources =
                new ComponentResourceManager(typeof(MainV2));
        ...
        public static DialogResult MessageShowAgain(string title, string promptText)
        {
            ...
            if (Settings.Instance.ContainsKey((string)chk.Tag) &&
                Settings.Instance.GetBoolean((string)chk.Tag) == false)
                // skip it
            {
                form.Dispose();
                chk.Dispose();
            ...
            if (link != "" && linktext != "")
            {
                var textSize2 = TextRenderer.MeasureText(linktext, SystemFonts.DefaultFont);
